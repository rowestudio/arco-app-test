...
## DEC-2026-08-23-01 — E9G: side width handles substituem o slider de largura; revisão deliberada de REG-028
...
- **Status:** ativa; correção de revisão aplicada (`index.html`, `tests/smoke/app.spec.mjs`); gate `E9G —` em `tests/smoke/app.spec.mjs` (TC-054) atualizado, **confirmação de WebKit 30/30 no CI ainda pendente** de nova execução (não executável neste ambiente), **validação física pendente** (iPhone/Safari, Roberto). Nenhuma promoção autorizada.
- **Documento relacionado:** `docs/REGRESSIONS.md` (REG-056, REG-055 atualizada para RESOLVIDA FISICAMENTE), `docs/PRODUCT_RULES.md` (Regra E9G), `docs/TEST_CASES.md` (TC-054), `docs/PROJECT_STATE.md`, `docs/ROADMAP.md`, `docs/PRE_PROMOTION_RELEASE_PLAN.md`.
...
- **Status:** ativa; implementada e automatizada (três gates `REG-055 —` após a R1), **validação física pendente** (iPhone/Safari, Roberto — terceira tentativa após duas reprovações). Nenhuma promoção autorizada; nenhum merge automático; nova revisão solicitada após a R1.
