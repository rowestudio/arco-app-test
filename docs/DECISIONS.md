...
- **Status:** ativa; correção de revisão aplicada (`index.html`, `tests/smoke/app.spec.mjs`); gate `E9G —` em `tests/smoke/app.spec.mjs` (TC-054) atualizado, **confirmação de WebKit verde no CI ainda pendente** (não executável neste ambiente), **validação física pendente** (iPhone/Safari, Roberto). Nenhuma promoção autorizada.
