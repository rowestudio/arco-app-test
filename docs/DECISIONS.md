...
## DEC-2026-08-24-01 — E9G1: nova tentativa de side width handles sobre a base restaurada, após validação física do rollback
...
- **Status:** implementação em PR no repositório de teste; validação automatizada (Chromium `hasTouch`, WebKit ausente no ambiente de implementação) registrada em `docs/PROJECT_STATE.md`. **Validação física completa em iPhone/Safari por Roberto permanece pendente** antes de REG-056/REG-057-sob-E9G1 serem consideradas resolvidas fisicamente. Nenhum merge automático; nenhuma promoção autorizada.
- **Documento relacionado:** `docs/REGRESSIONS.md` (REG-056, REG-057, REG-028), `docs/PRODUCT_RULES.md`, `docs/PROJECT_STATE.md`, `docs/ROADMAP.md`, `docs/PRE_PROMOTION_RELEASE_PLAN.md`, `docs/TEST_CASES.md` (TC-054), DEC-2026-08-23-01 (desenho técnico de referência, histórico), DEC-2026-08-23-02 (decisão de rollback).
