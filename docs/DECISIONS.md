...
## DEC-2026-08-22-02 — REG-055: wrapper/input do "+" tornam-se persistentes no DOM (correção de revisão da PR #509, v8z4b32E9F5-R2)

- **Data:** 2026-08-22. **Versão:** `v8z4b32E9F5` (mantida — segunda correção de revisão sobre a mesma PR #509, sem bump).
- **Assunto:** estabilidade no DOM do `input[type=color]` nativo introduzido como alvo de toque pela `DEC-2026-08-22-01` (R1), após revisão técnica apontar uma regressão introduzida pela própria R1.
- **Classificação:** correção de revisão da mesma PR funcional REG-055. Sem Engine/curvas/timing/renderer; sem Preview/Export; sem produção.
- **Contexto:** a R1 tornou o input nativo o alvo real do toque, mas o reposicionava via `row.innerHTML = …` seguido de `wrap.appendChild(input)` a cada chamada de `renderProjectBgSwatches()`/`renderTextColorSwatches()`/`renderTextBgSwatches()`. Como `setTextColor()`/`setTextBoxBackground()` chamam essas funções de render a cada evento `input` do PRÓPRIO picker nativo (disparado continuamente durante o gesto de escolha), o input ativo era removido do DOM e reinserido enquanto o usuário ainda podia estar interagindo com o picker — risco real de interromper a interação no iPhone/Safari. Checar apenas identidade de objeto (`===`) não detecta esse bug, pois `appendChild` reaproveita o MESMO objeto DOM (nunca clona) — por isso o comportamento passou despercebido na verificação inicial da R1.
- **Decisão A — wrapper/input tornam-se elementos ESTÁTICOS:** `#bgColorTriggerWrap`/`#bgHexInput`, `#textColorTriggerWrap`/`#textCreationColor` e `#textBgTriggerWrap`/`#textBoxBackgroundColor` passam a ser declarados uma única vez no HTML, como o último filho de cada linha de swatches. As três funções de render nunca mais tocam esses elementos.
- **Decisão B — render por substituição cirúrgica dos swatches, não por reconstrução total:** `renderProjectBgSwatches()`/`renderTextColorSwatches()`/`renderTextBgSwatches()` passam a remover apenas os swatches dinâmicos anteriores (presets/paleta pessoal/"Sem cor") e inserir os novos via `wrap.insertAdjacentHTML('beforebegin', …)` relativo ao wrapper estático — nunca via `row.innerHTML` do container inteiro.
- **Decisão C — prova de estabilidade no gate, não só de identidade:** o gate `REG-055` ganha `assertColorTriggerNeverRemounted()`, que usa `MutationObserver` no `childList` da linha de swatches para detectar se o wrapper/input aparecem em `removedNodes` durante a própria reação a `input`/`change` — capturando a remoção momentânea mesmo quando o objeto volta a ser idêntico depois. Verificado retroativamente: rodando este novo assert contra o código da R1 (antes desta correção), o teste falha corretamente (`removedDuringUpdate: true`), confirmando que a prova anterior (`elementFromPoint`/geometria) não era suficiente para pegar esta classe de regressão.
- **Preservado sem mudança:** toda a arquitetura de ativação direta da R1 (input nativo como alvo real do toque, ausência de `#customColorPanel`, ausência de `.click()` programático) e toda a lógica de aplicação/paleta pessoal da implementação inicial.
- **Alternativa rejeitada:** manter a reconstrução total via `row.innerHTML` e mitigar restaurando o foco/posição do scroll após o render — rejeitada por não eliminar a janela real de desconexão do DOM (ainda haveria um instante em que o input não está no documento), apenas mascarar sintomas secundários.
- **Status:** ativa; implementada e automatizada (gate `REG-055` com nova prova de estabilidade), validação física pendente (iPhone/Safari, Roberto). Nenhuma promoção autorizada.
- **Documento relacionado:** `docs/REGRESSIONS.md` (REG-055), `docs/PRODUCT_RULES.md` (regra canônica dos seletores de cor), `docs/TEST_CASES.md` (TC-053), `docs/PROJECT_STATE.md`.

## DEC-2026-08-22-01 — REG-055: ativação DIRETA do input[type=color] nativo (correção de revisão da PR #509, v8z4b32E9F5-R1)

- **Data:** 2026-08-22. **Versão:** `v8z4b32E9F5` (mantida — correção de revisão sobre a mesma PR #509, sem bump).
- **Assunto:** mecanismo de ATIVAÇÃO do "+" nos três seletores de cor, após revisão técnica da PR #509 apontar que a primeira rodada da E9F5 preservava exatamente o mecanismo já responsável pela falha física original de REG-055.
- **Classificação:** correção de revisão da mesma PR funcional REG-055. Sem Engine/curvas/timing/renderer; sem Preview/Export; sem produção.
- **Contexto:** a primeira rodada da `v8z4b32E9F5` corrigiu a arquitetura (nenhum painel intermediário, DEC-2026-08-21-01 abaixo), mas manteve `#bgHexInput`/`#textCreationColor`/`#textBoxBackgroundColor` como inputs **1×1 px, deslocados para fora da tela (`left:-9999px`) e `pointer-events:none`**, abertos apenas por um `input.click()` programático disparado pelo `onclick` do botão "+" (`openBgColorPicker()`/`openTextColorPicker()`/`openTextBgColorPicker()`). Esse é exatamente o mecanismo de ativação já em produção quando Roberto reproduziu fisicamente REG-055 — provar que `.click()` foi chamado por JavaScript não prova que o TOQUE do usuário chega ao picker nativo no iPhone/Safari.
- **Decisão A — o input nativo passa a ser o alvo físico real do toque:** o botão "+" visual permanece (agora `pointer-events:none`, puramente decorativo) e o MESMO `input[type=color]` já existente (nunca clonado, nunca recriado) é reposicionado a cada render sobre a área inteira do "+" via a primitiva `.color-trigger-wrap` (`position:absolute; inset:0; pointer-events:auto`), tornando-se o elemento que efetivamente recebe o toque/clique do usuário — comprovável por `document.elementFromPoint()` no centro do "+".
- **Decisão B — as funções de abertura por `.click()` são removidas, não mantidas como indireção morta:** `openBgColorPicker()`, `openTextColorPicker()` e `openTextBgColorPicker()` foram removidas do código — elas eram exatamente o mecanismo problemático, não uma indireção neutra a preservar por compatibilidade. Nenhuma ficou como código morto.
- **Decisão C (correção de framing de DEC-2026-08-21-01):** a referência anterior a esse padrão de `.click()` como "já aprovado" foi imprecisa — REG-055 é evidência física de que a ativação por `.click()` num input off-screen NÃO estava aprovada/funcional no iPhone/Safari. A regra canônica passa a ser explicitamente "+' aciona ativação DIRETA e Safari-safe do `input[type=color]` nativo", registrada em `docs/PRODUCT_RULES.md`.
- **Preservado sem mudança:** toda a lógica de APLICAÇÃO (`setBgColorHex`/`commitBgColorEdit`/`setBgColor`, `setTextColor`, `setTextBoxBackground`, paleta pessoal `customColorPalette`, HEX inline, Undo/dirty, "Sem cor"/opacidade do Fundo da caixa) e as Decisões A/C de `DEC-2026-08-21-01` (ausência de painel intermediário; paleta pessoal persistente compartilhada).
- **Alternativa rejeitada:** manter `.click()` programático e apenas "provar" via um spy que o clique foi disparado — rejeitada porque o gate anterior aprovava exatamente o mecanismo que falhava fisicamente; o gate `REG-055` foi reescrito para provar estruturalmente (bounding rect, `pointer-events` computado, `elementFromPoint`) que o input nativo é o alvo real.
- **Status:** ativa; implementada e automatizada (gate `REG-055` reescrito), validação física pendente (iPhone/Safari, Roberto), incluindo a abertura efetiva da UI nativa (roda/espectro/conta-gotas) quando disponibilizada pelo Safari/iOS — o headless não pode simular essa UI do sistema. Nenhuma promoção autorizada.
- **Documento relacionado:** `docs/REGRESSIONS.md` (REG-055), `docs/PRODUCT_RULES.md` (regra canônica dos seletores de cor), `docs/TEST_CASES.md` (TC-053), `docs/PROJECT_STATE.md`.
...
- **Decisão B — preservação explícita do picker completo existente (NOTA: framing corrigido por `DEC-2026-08-22-01` acima — o padrão `.click()` sobre input off-screen aqui descrito NÃO era "já aprovado"; era o próprio mecanismo causador da falha física, substituído na correção de revisão):** os três fluxos já existentes (`beginBgColorEdit`/`commitBgColorEdit`/`setBgColor`/`setBgColorHex`/`onBgHexText`; aplicação de Cor do texto/Fundo da caixa) permanecem como caminho canônico, intocados. O HEX é inline no painel já existente (novo em Cor do texto/Fundo da caixa; já existia no Fundo do projeto).
...
- **Status:** superada parcialmente por `DEC-2026-08-22-01` (mecanismo de ativação); Decisões A e C permanecem ativas. Validação física pendente (iPhone/Safari, Roberto), incluindo roda/espectro e conta-gotas do picker nativo quando disponibilizados pelo Safari/iOS. Nenhuma promoção autorizada.
