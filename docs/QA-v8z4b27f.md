...
- Menu principal de frames selecionados virou faixa horizontal rolável, com áreas de toque preservadas e prevenção de seleção/callout nativos no iPhone/Safari.
- Ícone de `Pausa` trocado para relógio/tempo; `Selecionar todos` trocado para `list-check`; `Reset`/`Zerar` em Escala/Rotação passa a usar o símbolo oficial `i-reset`.
...
6. Confirmar menu principal rolável horizontalmente, sem espremer itens e sem interferir na faixa de frames.
