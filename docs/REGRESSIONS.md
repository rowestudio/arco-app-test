...
- **Teste preventivo:** `tests/smoke/app.spec.mjs`, gate `REG-053 — painel de transformação em multi-seleção é invariante à ordem das ações (390×797)` (ver `docs/TEST_CASES.md` TC-052). Testa Rotação, Escala, Mover/Posição e Pausa; exige submenu real visível, sem painel concorrente, sem clipping, bounding rect dentro do viewport, sem invadir pills, Voltar acessível, e equivalência geométrica entre Fluxo A e Fluxo B. Falha comprovadamente na CSS pré-correção (validado nesta sessão revertendo a correção e reexecutando o gate) e passa após a correção.
