...
| REG-025 | Autosave/retomar sessão restaura estado não confiável. | Usuário continua de estado parcial ou incorreto. | Persistência/sessão | Acionar retomada de sessão e comparar estado esperado. | `node scripts/qa/check-session-autosave-restore.mjs` + fixture positiva/negativa; teste real iPhone/Safari. | automatizado parcial; Safari real pendente |
