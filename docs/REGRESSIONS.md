...
> Atualização 2026-08-21 (`v8z4b32E9F4`): **REG-053** foi **validada fisicamente por Roberto** (build `v8z4b32E9F3` publicada, PR #506, iPhone/Safari) e está **RESOLVIDA**. **REG-055** teve sua interpretação original ("abre o color picker nativo") **marcada como superada** por clarificação explícita de produto de Roberto (2026-08-21); a correção/aprimoramento (painel compartilhado de customização + paleta pessoal persistente) foi **implementada e automatizada na `v8z4b32E9F4`**, com **validação física pendente**. **REG-052** permanece **ABERTO** e **não corrigido**. **REG-054** teve a **correção implementada/automatizada na `v8z4b32E9F2`**, com validação física **PARCIAL** (ver abaixo) — não deve ser marcada resolvida integralmente sem o restante do checklist físico.

## REG-055 — Unificação da customização de cor + paleta pessoal persistente (implementada/automatizada; validação física pendente)
...
- **Interpretação original (SUPERADA):** "o botão `+` deveria abrir o color picker nativo". Essa formulação era estreita e foi **explicitamente superada** por clarificação de produto de Roberto em **2026-08-21**.
- **Clarificação de produto de Roberto (2026-08-21):** *"O seletor de cor deve salvar as cores customizadas pelo usuário."* O botão `+` significa **ADICIONAR/PERSONALIZAR COR** — deve abrir uma experiência de customização coerente com o painel já existente de Cor de Fundo do projeto, com no mínimo: preview da cor, campo hexadecimal, confirmação/aplicação explícita, e integração com uma **paleta pessoal persistente**. Um `input[type=color]` nativo isolado não é solução final (pode ser usado apenas como auxílio interno).
- **Implementação (`v8z4b32E9F4`):** painel único compartilhado `#customColorPanel` (reaproveita a estrutura visual `.float-panel`/`panel-handle`/`panel-title` do seletor de Fundo do projeto + o cabeçalho ×/✓ já aprovado do editor de Text Asset), usado pelos três contextos — `project-background`, `text-color`, `text-box-background` — sem criar três painéis independentes. No Fundo do projeto, o antigo trigger "Personalizar" (quadrado cinza `input[type=color]` + campo hexadecimal sempre visíveis) foi substituído pelo botão `+` (`.bg-swatch-add`), coerente com os `+` já existentes de Cor do texto/Fundo da caixa (E9F1), que passam a abrir esta MESMA experiência em vez de clicar um `input[type=color]` oculto.
  - **Campo hexadecimal:** aceita entrada com ou sem `#`, maiúsculas/minúsculas equivalentes; normalizado para uma representação canônica única `#rrggbb` minúsculo (`normalizeCustomColorHex`). Entrada inválida não aplica, não persiste, não cria swatch e não quebra o painel (feedback textual não técnico no próprio painel).
  - **Paleta pessoal persistente:** ao confirmar uma cor válida, ela é aplicada pelo caminho **canônico** do contexto de origem (`setBgColor`/`setTextColor`/`setTextBoxBackground`, sem alteração de semântica) e adicionada a `customColorPalette`, persistida em `localStorage['arco_user_custom_colors_v1']` (mesmo padrão defensivo try/catch já usado por `AUTOSAVE_KEY`/hints de modo). Sobrevive a fechar/reabrir o editor, a reload da página e a abrir outro projeto; compartilhada pelos três contextos; deduplicada por valor normalizado (`#ff6b8a` e `#FF6B8A` = 1 entrada); cores que coincidem com um neutro built-in de `PROJECT_BG_NEUTRALS` não geram swatch redundante; entradas inválidas/corrompidas no storage são ignoradas sem quebrar o app, e os neutros built-in continuam disponíveis mesmo se a preferência não carregar.
  - **Não integra o projeto:** a paleta pessoal **NÃO** entra em `buildProjectData`/JSON do projeto/Save-Load/Session Restore/ProjectWorld/Text Asset — é preferência local do usuário no navegador, sem sync/nuvem/conta.
  - **Cancelar/abrir sem confirmar:** abrir o painel não aplica nada por si só; digitar sem confirmar não persiste; cancelar (×/alça) descarta a edição sem criar swatch fantasma, sem Undo e sem autosave.
  - **Preservado por contexto:** Cor do texto continua alterando somente os glifos do `pendingTextDraft` (WYSIWYG imediato, sem Undo prematuro); Fundo da caixa continua ligando `boxBackgroundEnabled`/revelando a opacidade já existente (que afeta somente o fundo); Fundo do projeto continua usando `setBgColor`/`getProjectBackgroundColor()` (Undo/dirty/restart de Preview preservados); `DEFAULT_PROJECT_BG` (`#3c3c3b`) intocado.
- **Classificação:** aprimoramento funcional de UX + correção/clarificação da REG-055 (interface de cores, persistência local, WYSIWYG de Text Asset).
- **Independência:** não corrige REG-052 (fill visual dos sliders), REG-053 (painel cortado da multi-seleção, já resolvida — ver abaixo) nem a matemática/targets de REG-054 (multi-seleção de Frames).
- **Teste preventivo:** `tests/smoke/app.spec.mjs`, gate `E9F4 — REG-055: painel compartilhado de customização de cor e paleta pessoal persistente` (ver `docs/TEST_CASES.md` TC-053).
- **Status:** **correção/aprimoramento implementado e automatizado** na `v8z4b32E9F4` (PR própria). Validação física em iPhone/Safari **pendente** — Roberto testará a build publicada nos três contextos (Fundo do projeto, Cor do texto, Fundo da caixa), incluindo persistência entre reload e entre projetos. **Somente Roberto poderá fechar fisicamente a REG-055** após esse teste. Nenhuma promoção autorizada.
...
## REG-053 — Painel de transformação com multi-seleção é cortado (RESOLVIDA fisicamente)
...
- **Status:** **RESOLVIDA.** Correção implementada e automatizada na `v8z4b32E9F3` (PR #506, merge commit `67661047f822ba8434783b862af9f31b502d6d76`); build publicada no repositório de teste; **Roberto testou fisicamente em iPhone/Safari e aprovou visualmente** a correção (2026-08-20/21) — painel de transformação em multi-seleção deixou de depender da ordem das ações, Voltar/slider/chips/timeline/Stage/seleção/Undo/Redo e a transformação REG-054 preservados nos dois fluxos. Nenhuma promoção autorizada.
