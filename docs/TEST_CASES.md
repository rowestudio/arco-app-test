...
## TC-053 — Picker completo preservado, HEX inline e paleta pessoal persistente e compartilhada (REG-055)

Gate `tests/smoke/app.spec.mjs`, `REG-055 — picker completo preservado, HEX inline e paleta pessoal persistente e compartilhada`, viewport iPhone **390 × 797**. Fluxo 100% público nos três painéis de cor (Fundo do projeto, Cor do texto, Fundo da caixa) — nenhuma chamada direta às funções internas de aplicação de cor estabelece o estado testado; a exceção declarada é o disparo de eventos DOM reais (`input`/`change`) diretamente nos elementos `input[type=color]`/`input[type=text]` já existentes, no mesmo padrão já usado pelo helper `enableTextBoxColor` desta suíte, já que o picker OS-nativo não pode ser pilotado por um agente headless. Falha na base pré-`v8z4b32E9F5` (sem HEX inline em Cor do texto/Fundo da caixa, sem `customColorPalette`/`arco_user_custom_colors_v1`) e passa após a correção.
...
- **Teste A (Fundo do projeto):** abre o painel pelo fluxo público real (segundo toque em `#modeAssetsBtn` já ativo → menu contextual do Modo Ativos → botão "Fundo"); confirma presets em `#bgSwatches`, `#bgHexText` visível e um botão `.bg-swatch-add` visível; tocar nesse "+" aciona um clique real no MESMO `#bgHexInput` nativo (espionado via `addEventListener`), sem alterar a paleta por si só e sem criar `#customColorPanel`.
- **Teste B (Cor do texto):** cria um Text Asset pelo fluxo público (`#lowerAddOrSelectAllBtn` → `#assetsMenuTextBtn`), abre a aba "Cor do texto"; confirma `#textColorHexText` visível e um botão `.text-swatch-add` visível; tocar nesse "+" aciona um clique real no `#textCreationColor` nativo (mesmo `openTextColorPicker()` já existente).
- **Teste C (Fundo da caixa):** aba "Fundo da caixa" preserva "Sem cor" ativo por padrão (`boxBackgroundEnabled=false`, `#textEditorBgOpacityWrap` oculto); `#textBoxBackgroundHexText` visível; "+" aciona um clique real no `#textBoxBackgroundColor` nativo (`openTextBgColorPicker()`); escolher HEX válido liga o fundo e revela a opacidade; voltar a "Sem cor" desliga o fundo, oculta a opacidade e preserva a cor dos glifos.
- **Teste D (escolha pelo picker):** despachar `input`+`change` reais no `input[type=color]` correspondente aplica a cor ao contexto certo e a cor entra em `customColorPalette` com `lastCustomColorSource === 'picker'`.
...
- **Teste F (inválido):** HEX inválido não aplica, não salva e não cria swatch — verificado nos três contextos.
...
- **Teste K (cancelamento):** abrir o picker sem efetivar escolha (nenhum evento `change` subsequente) não altera o tamanho da paleta; focar/digitar HEX incompleto ou inválido e abandonar (com ou sem `change`) não adiciona swatch.
- **Não altera:** REG-052 (fill dos sliders), REG-053 (TC-052 continua passando), REG-054 (TC-051 continua passando), `boxBackgroundEnabled`/opacidade do Fundo da caixa, Undo/Redo, Preview/Export, ProjectWorld, Save/Load.
- **Ambiente:** WebKit é o gate obrigatório (checks do HEAD final); verificação equivalente local executada em **Chromium `hasTouch`** nesta sessão (binário pré-instalado do ambiente; download do WebKit bloqueado pelo proxy de rede local). Suíte completa de `tests/smoke/app.spec.mjs` executada sem regressão nova. Validação física obrigatória em iPhone/Safari **pendente** (Roberto, após merge e publicação da build), incluindo roda/espectro e conta-gotas do picker nativo quando disponibilizados pelo Safari/iOS.
