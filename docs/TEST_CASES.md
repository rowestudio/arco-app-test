...
## TC-055 — Affordance compacto + sheet parcial de Camadas (E9H)

- Pré-condição: Modo Ativos, fixture real carregada, viewport 390 × 797.
- Passos: (a) verificar `#layersAffordance` ausente no Modo Câmera/Frames, presente no Modo Ativos, ausente em `body.previewing`/`body.recording`; (b) criar uma segunda Layer (texto) e abrir a sheet pelo affordance; (c) medir a altura de `.layers-sheet-inner` contra ~40% do viewport e confirmar que `#stage` continua com altura visível; (d) conferir que cada linha mostra thumbnail/ícone, nome (`getAssetDisplayName`), posição na pilha e profundidade (`Posição N de M · Prof. ±X`); (e) tocar numa Layer diferente da selecionada e confirmar que `selectedAssetId` muda e a sheet permanece com a classe `show`; (f) alternar visibilidade pelo botão da linha e confirmar `asset.visible`; (g) usar Subir/Descer numa linha e confirmar que só `zIndex` muda, nunca `depth`; (h) confirmar que "Trocar" só existe na linha de asset do tipo imagem; (i) excluir uma Layer pelo botão da linha e confirmar que `deleteSelectedAsset()` foi exercitada (asset removido, contagem de linhas atualizada); (j) fechar pela alça/`×` e confirmar `aria-hidden="true"`; (k) criar 8+ Layers com nome longo e confirmar `overflow-y:auto` em `#layersList` com `scrollHeight > clientHeight` e `text-overflow:ellipsis` no nome.
- Resultado esperado: affordance aparece somente no contexto correto (Modo Ativos, fora de Preview/Export) e não reutiliza `#stageEyeShortcut`; sheet nunca excede ~40% do viewport nem vira modal full-screen; Stage permanece visível/interativo acima; seleção de Layer altera a seleção canônica sem fechar a sheet; fechamento só ocorre por ação explícita; visibilidade, frente/trás, trocar e excluir preservam integralmente o comportamento das funções canônicas já existentes (`selectAssetById`, `layerMoveAssetUp`/`layerMoveAssetDown`, `layerReplaceImageForAsset`, `deleteSelectedAsset`); lista suporta overflow/scroll interno sem quebrar o layout em 390 × 797.
- Evidência: gates `E9H —` em `tests/smoke/app.spec.mjs`. Execução local bloqueada pela ausência de acesso de rede para instalar WebKit/Chromium nesta sessão; depende do CI (Browser/WebKit Smoke Tests) e de validação física em iPhone/Safari por Roberto.

## TC-056 — Régua visual e botões `−10`/`+10` de Profundidade (E9H)

- Pré-condição: Modo Ativos, asset selecionável carregado, viewport 390 × 797.
- Passos: (a) abrir o painel de Profundidade (`#tbAssetDepth`) e confirmar `min=-100`/`max=100` no slider e o título "Profundidade" visível; (b) inspecionar `#assetContextDepthRuler` e confirmar exatamente três ticks com número (`−100`, `0`, `+100`) e oito ticks sem número, com o tick de `0` marcado como mais destacado (`depth-tick-zero`); (c) mover o slider para um valor não múltiplo de 10 (ex.: 37) e confirmar que é aceito sem arredondamento/snap; (d) medir `--fill-a`/`--fill-b` do slider para um valor positivo e um negativo, confirmando preenchimento a partir do zero em cada direção; (e) confirmar que a cor do preenchimento é `var(--accent)` = `#FF6B8A` no workspace Ativos; (f) clicar `−10`/`+10` e confirmar variação de exatamente 10, clamp nos extremos `−100`/`+100`, e exatamente 1 entrada de Undo por clique; (g) clicar Reset e confirmar retorno a 0 sem alterar `zIndex`; (h) simular um gesto de arraste contínuo (`pointerdown` + múltiplos valores + `change`) e confirmar exatamente 1 entrada de Undo para todo o gesto; (i) alterar `depth`, serializar via `buildProjectData`/`applyProjectData` e confirmar que o valor sobrevive ao round-trip.
- Resultado esperado: intervalo público contínuo `−100..+100` preservado sem snap pelas marcas; régua mostra somente três números de referência com oito ticks intermediários discretos; preenchimento bidirecional a partir do zero no acento coral vigente de Ativos, sem reintroduzir roxo; botões `−10`/`+10` (substituem a formulação anterior de `−5`/`+5`) e Reset seguem a mesma semântica de alteração única/confirmada já usada por Escala/Rotação; `depth` permanece independente de `zIndex`; Undo/autosave não multiplicam durante gestos intermediários; valores permanecem finitos; Save/Load preserva o valor.
- Evidência: gates `E9H —` em `tests/smoke/app.spec.mjs`. A cobertura básica de persistência/independência de Profundidade (Save/Load completo, Undo/Redo, finitude, paridade Stage/Preview/Export) já existia em `E9A —` (ver acima) e não foi duplicada aqui. Execução local bloqueada pela ausência de acesso de rede para instalar WebKit/Chromium nesta sessão; depende do CI e de validação física em iPhone/Safari por Roberto.
