...
- Passos: (a) criar texto de um único caractere pelo fluxo público; (b) editar para múltiplas linhas de larguras diferentes; (c) em **Largura da caixa**, travar uma largura menor (a partir da `v8z4b32E9G`, pela side width handle no Stage — ver TC-054; antes dela, pelo slider já superado) e concluir; executar Undo/Redo e Save/Load; voltar para **Auto**; (d) aplicar profundidade não-zero (positiva e negativa) pelo painel público.
...
- Evidência: gate E9C em `tests/smoke/app.spec.mjs` (WebKit Smoke Tests), adaptado na `v8z4b32E9G` para usar a side width handle em vez do slider superado. Execução local do gate em WebKit permanece bloqueada pela ausência do WebKit no ambiente; verificação equivalente executada em Chromium `hasTouch`. Validação final em iPhone/Safari real permanece pendente.

## TC-054 — Side width handles do Text Asset e correção de REG-056 (E9G)

- Pré-condição: Modo Ativos, fixture real carregada, viewport 390 × 797.
- Passos: (a) contar alças de um Asset de imagem e de um Text Asset selecionados; (b) preparar o Text Asset em Fixed com `textBaseBoxWidth` no meio da faixa lógica (folga real nos dois sentidos) e comparar escalar pelas corner handles com um width drag pela side handle direita; (c) executar um gesto direto de width drag em um Text Asset confirmado fora do editor; (d) disparar `pointercancel` a meio de um width drag; (e) executar Undo/Redo após um width drag confirmado; (f) rotacionar o Text Asset (45°) e repetir o width drag; (g) escalar o mesmo Text Asset para 50%, 100% e 200% e, em cada escala, arrastar a side handle até os limites lógicos mínimo e máximo; (g2) nesses mesmos limites, tentar arrastar além do limite (extrapolar); (h) ativar fundo/padding e repetir o width drag; (i) Save/Load após um width drag; (j) criar um Text Asset de uma linha em escala pequena e auditar a geometria das seis alças; (k) auditar Preview quanto à ausência das side width handles.
- Resultado esperado: (a) imagem = exatamente 4 corner handles + 0 side width handles; Text Asset = exatamente 4 corner handles + 2 side width handles (`data-asset-width-handle="left"/"right"`); (b) corner altera `fontSize`/`boxWidth` proporcionalmente (baseline intocado); side NUNCA altera `fontSize`/`textBaseFontSize` e preserva `getAssetContextScalePercent()` dentro de tolerância subpixel, e o primeiro delta efetivo entra em `boxWidthMode:'fixed'`; (c) o gesto completo (pointerdown→pointermove×N→pointerup) produz exatamente 1 Undo e exatamente 1 revisão de autosave, nunca um por `pointermove`; (d) `pointercancel` restaura escala/largura/posição/modo por completo, sem Undo/autosave, com o diagnóstico `textWidthPointerCancelRestored:true`; (e) Undo/Redo restauram e reaplicam largura/posição/`boxWidthMode` byte/valor-equivalentes; (f) rotação e `fontSize`/escala permanecem intocados, e a lateral oposta ao lado arrastado permanece ancorada no World (tolerância subpixel), mesmo sob rotação não trivial; (g) o mesmo Text Asset atinge o MESMO `textBaseBoxWidth` no limite lógico mínimo e no máximo (`getTextWidthLogicalBounds()`) nas três escalas — a faixa lógica de largura disponível não diminui simplesmente porque o ativo foi escalado (gate conceitual da REG-056); (g2) um gesto que tentaria extrapolar o limite já atingido é um NO-OP: `boxWidth`/`textBaseBoxWidth`/`worldX`/`worldY`/`worldW`/`worldH`/`boxWidthMode` permanecem byte/valor-equivalentes e zero Undo/zero revisão de autosave são criados — distinto do caso (b)/(g), que é um gesto válido com delta real; (h) `worldW = boxWidth + 2·paddingX` permanece coerente, padding/cor/opacidade do fundo não mudam, e o retângulo de fundo acompanha o retângulo de seleção sem descolar; (i) `boxWidth`/`textBaseBoxWidth`/`boxWidthMode`/`fontSize`/`textBaseFontSize`/`rotation`/posição sobrevivem a Save/Load; (j) as seis alças permanecem com área de toque não nula e sem overflow horizontal da página; (k) nenhuma side width handle aparece dentro de `#previewScreen`.
- Evidência: gate `E9G —` em `tests/smoke/app.spec.mjs` (WebKit Smoke Tests). Revisão R1 (mesma PR #512, HEAD anterior `ed384f0`) corrigiu dois defeitos reais de prova/implementação de suporte encontrados pelo WebKit no HEAD revisado — uma variável Node referenciada indevidamente dentro de `page.evaluate()` no gate E9C, e `endTextWidthPointer()` marcando dirty/agendando autosave fora do bloco `if(changed)` mesmo em gestos absorvidos pelo clamp (`index.html`) — além de corrigir o setup do passo (b) para garantir folga real de largura. Execução local do gate em WebKit permanece bloqueada pela ausência do WebKit no ambiente (rede restrita ao proxy do agente, sem acesso ao CDN de download dos binários do Playwright, confirmado novamente na revisão R1); nenhuma verificação equivalente em Chromium foi possível pelo mesmo motivo. A confirmação de que o WebKit Smoke Tests passa integralmente no novo HEAD depende do CI. Validação final em iPhone/Safari real permanece pendente, incluindo o protocolo físico completo descrito na tarefa.
- Automatizável: sim, exceto aprovação visual final e teste físico em iPhone/Safari.
