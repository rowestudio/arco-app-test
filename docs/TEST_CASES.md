...
## TC-053 — "+" aciona diretamente o input[type=color] nativo (alvo real de toque), HEX inline e paleta pessoal persistente e compartilhada (REG-055)

Gate `tests/smoke/app.spec.mjs`, `REG-055 — "+" aciona diretamente o input[type=color] nativo (alvo real de toque), HEX inline e paleta pessoal persistente e compartilhada`, viewport iPhone **390 × 797**. Fluxo 100% público nos três painéis de cor (Fundo do projeto, Cor do texto, Fundo da caixa) — nenhuma chamada direta às funções internas de aplicação de cor estabelece o estado testado; a exceção declarada é o disparo de eventos DOM reais (`input`/`change`) diretamente nos elementos `input[type=color]`/`input[type=text]` já existentes, no mesmo padrão já usado pelo helper `enableTextBoxColor` desta suíte, já que um agente headless não pode pilotar a UI do picker do sistema operacional. Falha na base pré-`v8z4b32E9F5-R1` (inputs 1×1 px, deslocados para fora da tela, `pointer-events:none`, abertos só por `.click()` programático) e, com a prova de estabilidade da R2, falha também contra o código da própria R1 (input reconstruído a cada evento próprio) — passa somente após a correção R2.

**O QUE ESTE GATE REALMENTE PROVA (e o que não prova):** a automação prova ESTRUTURALMENTE, nos três contextos, que o próprio `input[type=color]` nativo — não o botão "+" visual, não nenhum painel/modal — é o alvo físico direto do toque: geometria do input idêntica à área do "+" (não 1×1, não fora da tela), `pointer-events` computado diferente de `none`, e `document.elementFromPoint()` no centro tocável do "+" resolvendo exatamente para esse input. Prova também, via `MutationObserver`, que esse input (e seu wrapper) NUNCA são removidos do DOM ao reagir aos seus próprios eventos `input`/`change` — checar só identidade de objeto (`===`) não bastaria, pois `appendChild` reaproveita o MESMO objeto DOM mesmo quando o remove e reinsere. Também prova que despachar `input`/`change` reais nesse input aplica a cor corretamente e salva na paleta pessoal nos três contextos. **O gate NÃO prova, e não pode provar** em ambiente headless, que a UI nativa do picker do Safari/iOS (roda/espectro/conta-gotas) efetivamente abre, nem que uma interação real e contínua com essa UI (arrastar na roda, usar o conta-gotas) sobrevive sem interrupção — isso permanece validação física obrigatória pós-merge (ver seção "Ambiente" abaixo).

- **Ausência do painel rejeitado (Teste L):** `document.getElementById('customColorPanel') === null`, verificado no início e repetidamente após cada "+" tocado nos três painéis.
- **Prova estrutural do alvo de toque (Bloqueador 1, Testes A/B/C):** para cada um dos três "+" (`#bgSwatches`, `#textColorSwatches`, `#textBgSwatches`), após `scrollIntoViewIfNeeded()` (as linhas de swatches rolam horizontalmente e o "+" é o último item) e aguardar a transição CSS de abertura do painel/sheet terminar: o input correspondente (`#bgHexInput`/`#textCreationColor`/`#textBoxBackgroundColor`) é filho do wrapper ESTÁTICO do "+" (`#bgColorTriggerWrap`/`#textColorTriggerWrap`/`#textBgTriggerWrap`, classe `.color-trigger-wrap`), sua `getBoundingClientRect()` coincide (tolerância 1,5 px) com a do wrapper, não é 1×1 px, não está fora da tela, seu `pointer-events` computado não é `none`, e `document.elementFromPoint()` no centro do wrapper resolve exatamente para esse `<input type="color">` — nunca para o botão visual (`pointer-events:none`) nem para outro elemento.
- **Prova de estabilidade do DOM (correção R2, `assertColorTriggerNeverRemounted`):** embutida nas três provas de "escolha pelo picker" (Teste D) abaixo. Antes de despachar `input`+`change` no input nativo de cada contexto, instala um `MutationObserver` no `childList` (com `subtree`) da linha de swatches; depois do despacho, confirma que o wrapper/input NUNCA apareceram em `removedNodes` de nenhuma mutação, que o objeto DOM do input/wrapper é o MESMO antes/depois, que `input.isConnected === true`, e que `input.parentElement` continua sendo o MESMO wrapper. Detecta especificamente o padrão `row.innerHTML = …` + `appendChild(mesmoInput)` que a correção R1 usava — verificado retroativamente: rodando este assert contra o código da R1 (commit `eca607c`) o teste falha com `removedDuringUpdate: true` para Fundo do projeto, confirmando que a prova de geometria/`elementFromPoint` sozinha não capturava esta classe de regressão.
- **Teste A (Fundo do projeto):** abre o painel pelo fluxo público real (segundo toque em `#modeAssetsBtn` já ativo → menu contextual do Modo Ativos → botão "Fundo"); confirma presets em `#bgSwatches` e `#bgHexText` visível, além da prova estrutural acima para `#bgHexInput`.
- **Teste B (Cor do texto):** cria um Text Asset pelo fluxo público (`#lowerAddOrSelectAllBtn` → `#assetsMenuTextBtn`), abre a aba "Cor do texto"; confirma `#textColorHexText` visível, além da prova estrutural acima para `#textCreationColor`.
- **Teste C (Fundo da caixa):** aba "Fundo da caixa" preserva "Sem cor" ativo por padrão (`boxBackgroundEnabled=false`, `#textEditorBgOpacityWrap` oculto); `#textBoxBackgroundHexText` visível; prova estrutural acima para `#textBoxBackgroundColor`; escolher HEX válido liga o fundo e revela a opacidade; HEX inválido não muda `boxBackgroundEnabled`/`boxBackgroundColor`/paleta; voltar a "Sem cor" desliga o fundo, oculta a opacidade e preserva a cor dos glifos.
- **Teste D (escolha pelo picker, nos TRÊS contextos):** despachar `input`+`change` reais no PRÓPRIO `input[type=color]` de cada contexto (após `.focus()`, replicando a ordem real do gesto) aplica a cor corretamente: Fundo do projeto muda `bgColor` e cria exatamente 1 entrada de Undo; Cor do texto muda `pendingTextDraft.color` SEM alterar `boxBackgroundEnabled`/`boxBackgroundColor`; Fundo da caixa habilita o fundo, muda `boxBackgroundColor`, revela a opacidade e preserva `pendingTextDraft.color`. Nos três, a cor entra em `customColorPalette` com `lastCustomColorSource === 'picker'` e `lastCustomColorContext` igual a `'bg'`/`'text-color'`/`'text-box-background'`.
- **Teste E (escolha por HEX):** HEX inline válido (com ou sem `#`, maiúsculo ou minúsculo) aplica ao contexto (Fundo do projeto, glifos do texto, ou fundo da caixa habilitando-o) e entra na mesma paleta com `lastCustomColorSource === 'hex'`.
- **Teste F (inválido, nos TRÊS contextos):** HEX inválido não aplica, não salva e não cria swatch — verificado explicitamente em Fundo do projeto, Cor do texto e Fundo da caixa (`boxBackgroundEnabled`/`boxBackgroundColor` inalterados nesse último).
- **Teste G (persistência):** após adicionar cores pelo picker e por HEX, um novo carregamento do app (`page.goto` equivalente a reload, com `clearStartupStorage` ANTES e DEPOIS para evitar o diálogo de recuperação de sessão sem tocar `localStorage`) preserva `customColorPalette`; abrir outro projeto (mesmo arquivo, fluxo público de `#projectFileInput`) preserva a mesma paleta e ela aparece novamente como swatch no painel de Fundo.
- **Teste H (compartilhamento):** uma cor salva pelo Fundo do projeto aparece como swatch pessoal em Cor do texto e Fundo da caixa; uma cor salva por HEX em Cor do texto aparece em Fundo da caixa.
- **Teste I (deduplicação):** `#ff6b8a`/`FF6B8A`/`ff6b8a` sem `#` resultam em uma única entrada lógica na paleta (`customColorDuplicatePrevented === true` na repetição).
- **Teste J (projeto):** `JSON.stringify(buildProjectData(true))` não contém nenhuma das cores pessoais não utilizadas pelo projeto nem as strings `arco_user_custom_colors`/`customColorPalette`.
- **Teste K (cancelamento):** a mera existência/geometria do input (sem despachar `change`) não altera o tamanho da paleta; focar/digitar HEX incompleto ou inválido e abandonar (com ou sem `change`) não adiciona swatch.
- **Não altera:** REG-052 (fill dos sliders), REG-053 (TC-052 continua passando; RESOLVIDA fisicamente, sem relação com REG-055), REG-054 (TC-051 continua passando; PARCIAL, sem relação com REG-055), `boxBackgroundEnabled`/opacidade do Fundo da caixa, Undo/Redo, Preview/Export, ProjectWorld, Save/Load.
- **Ambiente:** WebKit é o gate obrigatório (checks do HEAD final); verificação equivalente local executada em **Chromium `hasTouch`** nesta sessão (binário pré-instalado do ambiente; download do WebKit bloqueado pelo proxy de rede local). Suíte completa de `tests/smoke/app.spec.mjs` executada sem regressão nova. **Validação física obrigatória em iPhone/Safari permanece pendente** (Roberto, após merge e publicação da build) especificamente para a abertura efetiva da UI nativa do picker (roda/espectro/conta-gotas do Safari/iOS quando disponibilizados) — este gate prova a geometria/interatividade do alvo de toque, não a experiência visual do sistema operacional.
