...
## TC-054 — Side width handles do Text Asset e correção de REG-056 (E9G1) — GATE OBRIGATÓRIO, IMPLEMENTADO NA `v8z4b32E9G1`

> **Estado (2026-08-24):** a `v8z4b32E9G1` (nova tentativa, sobre a base restaurada `v8z4b32E9F6`) implementa este gate em `tests/smoke/app.spec.mjs` (testes `E9G1 —` e `TC-054 —`) e o código correspondente em `index.html`. A primeira tentativa (`v8z4b32E9G`, PR #512) havia implementado um gate equivalente, mas toda a implementação foi revertida após reprovação física por REG-057; o rollback foi validado fisicamente (REG-057 resolvida fisicamente na base restaurada — ver `docs/REGRESSIONS.md`). Esta é uma implementação NOVA, não uma reversão do revert.

- Pré-condição: Modo Ativos, fixture real carregada, viewport 390 × 797.
- Passos (a)–(m) — cobertos pelo gate `E9G1 —`: contar alças de imagem (4+0) vs. Text Asset (4+2); comparar corner scale × side width drag (fontSize/textBaseFontSize preservados pelo side drag, inclusive empilhado sobre um corner drag sem Undo); gesto direto fora do editor = exatamente 1 Undo + 1 autosave; `pointercancel` restaura integralmente (0 Undo/0 autosave); Undo/Redo restauram largura/centro/modo; rotação ~45° + width drag mantém a lateral oposta ancorada no World; escalar 50/100/200% e verificar mesma faixa lógica (`textBaseBoxWidth`) nos limites (REG-056); tentar extrapolar o limite em ambos os sentidos = no-op (0 Undo/0 dirty/0 autosave, sem mudança espúria de modo); fundo/padding acompanham o reflow sem descolar; Save/Load preservam `boxWidth`/`textBaseBoxWidth`/`boxWidthMode`/rotação/posição; Text Asset de uma linha em escala pequena mantém as seis alças alcançáveis em 390×797; ausência das side width handles em Preview.
- **Requisito adicional obrigatório, herdado da REG-057** — coberto pelo gate `TC-054 —`: a sequência completa de REEDIÇÃO após Preview, pelas DUAS rotas públicas de reedição existentes (toolbar "Editar"; duplo toque): criar/selecionar Text Asset → Editar → alterar → Confirmar → Preview → sair do Preview → selecionar/reutilizar o MESMO Text Asset → Editar novamente → **o editor deve abrir no mesmo ID** → alterar novamente → Confirmar → **sem criar asset duplicado**, repetido em DOIS ciclos por rota. Nenhuma das flags de diagnóstico (`textEditorOpen`, `textEditorMode`, `textEditorTargetAssetId`, `pendingTextCreationActive`) fica presa em estado inativo após o ciclo — verificado no gate.
- **Terceiro ciclo específico da E9G1 (GATE REG-057 COM E9G1 ATIVA)** — também coberto pelo gate `TC-054 —`: Editar → side width drag → Confirmar → Preview → sair do Preview → reabrir o MESMO Text Asset → side width drag novamente → Confirmar → Preview novamente. Verificado: mesmo asset ID em todo o ciclo; nenhuma layer duplicada; editor abre nas duas vezes; a largura lógica do segundo drag parte do estado confirmado no primeiro; side handles continuam funcionais depois do Preview; seleção canônica aponta para o mesmo Text Asset; nenhum estado de gesto fica preso; Preview continua funcionando.
- Resultado obtido: os dois gates (`E9G1 —`, `TC-054 —`) passam em verificação equivalente **Chromium `hasTouch`** (WebKit ausente no ambiente de implementação); ver `docs/PROJECT_STATE.md` para os resultados específicos desta PR.
- Evidência ainda pendente: WebKit Smoke Tests verde no CI da PR, e validação física completa em iPhone/Safari por Roberto — incluindo explicitamente a reedição repetida do mesmo Text Asset depois de sair do Preview e o ciclo com side width drag.
