...
> **Estado (2026-08-23):** a `v8z4b32E9G` (PR #512) implementou este gate em `tests/smoke/app.spec.mjs` e o código correspondente em `index.html`. A build publicada foi REPROVADA FISICAMENTE por REG-057 e toda a implementação (produto + guardrails + testes, incluindo este gate) foi **revertida** por rollback. Este TC-054 é preservado como especificação do gate obrigatório para uma FUTURA nova tentativa da E9G — não descreve comportamento atual do código, que não possui side width handles nem este gate nesta PR de rollback.
...
- **Requisito adicional obrigatório para a futura nova tentativa (descoberto pela REG-057, 2026-08-23):** além dos passos acima, o gate deve provar a sequência completa de REEDIÇÃO após Preview, pelas DUAS rotas públicas de reedição existentes (toolbar "Editar"; duplo toque): criar/selecionar Text Asset → Editar → alterar → Confirmar → Preview → sair do Preview → selecionar/reutilizar o MESMO Text Asset → Editar novamente → **o editor deve abrir no mesmo ID** → alterar novamente → Confirmar → **sem criar asset duplicado**. Nenhuma das flags de diagnóstico (`textEditorOpen`, `textEditorMode`, `textEditorTargetAssetId`, `pendingTextCreationActive`) pode ficar presa em estado inativo após esse ciclo.
- Resultado esperado: idêntico ao especificado na implementação revertida (ver histórico em `docs/DECISIONS.md`, DEC-2026-08-23-01), acrescido do requisito de reedição pós-Preview acima, que deve passar nas duas rotas públicas antes de qualquer nova tentativa ser considerada fisicamente aprovável.
- Evidência exigida da futura tentativa: gate `E9G —` (ou equivalente) em `tests/smoke/app.spec.mjs`, WebKit Smoke Tests verde no CI, e validação física completa em iPhone/Safari por Roberto — incluindo explicitamente a reedição repetida do mesmo Text Asset depois de sair do Preview.
