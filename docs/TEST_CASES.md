...
## TC-053 — Painel compartilhado de customização de cor + paleta pessoal persistente (REG-055/E9F4)

Gate `tests/smoke/app.spec.mjs`, `E9F4 — REG-055: painel compartilhado de customização de cor e paleta pessoal persistente`, viewport iPhone **390 × 797**. Fluxo público real: Fundo do projeto via segundo toque em `#modeAssetsBtn` (já em Ativos) → botão "Fundo" do menu contextual (`runAssetsContextMenuAction('background')` → `openPanel('BgColor')`); Cor do texto/Fundo da caixa via `startTextCreation()` + rail iconográfica real (`railTool`).

- **Projeto — botão `+`:** `#bgSwatches .bg-swatch` mantém exatamente as 11 cores de `PROJECT_BG_NEUTRALS` (sem duplicação); `#bgHexInput`/`#bgHexText` (antigo trigger "Personalizar") não existem mais no DOM; `#bgSwatches .bg-swatch-add` existe, é visível e, ao ser tocado, abre `#customColorPanel` com `customColorPanelContext==='project-background'`; abrir não cria Undo.
- **Hexadecimal inválido:** digitar um valor inválido mantém `customColorHexValid===false`, `#customColorConfirmBtn` desabilitado, `bgColor`/paleta inalterados.
- **Cancelar:** digitar um hex válido e cancelar (botão × do painel) não aplica a cor, não persiste na paleta, não cria swatch (`.bg-swatch-custom`) e não cria Undo; o painel de origem (`#panelBgColor`) permanece aberto.
- **Hexadecimal válido + confirmar:** normaliza maiúsculo→minúsculo; aplica pelo caminho canônico (`bgColor`/`getProjectBackgroundColor()` refletem o novo valor; `DEFAULT_PROJECT_BG` inalterado); cria exatamente 1 Undo (mesmo fluxo de um swatch neutro); persiste em `customColorPalette`; reabrir o painel mostra o novo `.bg-swatch-custom[data-custom-color]` com `active`.
- **Deduplicação:** confirmar a mesma cor sem `#`/em minúsculo não cria segunda entrada; `customColorDuplicatePrevented===true`.
- **Save/Load schema:** `buildProjectData(true)` não contém `customColorPalette`/`userCustomColors`/`customColors` nem qualquer chave que corresponda a `/customcolor/i`.
- **Compartilhamento entre contextos:** a cor criada no Fundo do projeto aparece como swatch em `#textColorSwatches` e `#textBgSwatches` do editor de Text Asset sem interação adicional; criar uma cor em Cor do texto (via `+`) altera **somente** `pendingTextDraft.color` (glifos, WYSIWYG imediato no Stage) preservando `boxBackgroundColor`/`boxBackgroundEnabled`; a mesma cor aparece imediatamente no swatch de Fundo da caixa; criar uma cor em Fundo da caixa liga `boxBackgroundEnabled`, aplica a cor e revela a opacidade (que afeta só o fundo); voltar a "Sem cor" desliga o fundo e preserva a cor dos glifos. Nenhuma dessas interações cria `textEditorUndoEntriesCreated` — só a confirmação normal do editor (✓) cria Undo/autosave do Text Asset.
- **Storage corrompido:** `localStorage['arco_user_custom_colors_v1']` com entradas inválidas mistas (`"#not-a-color"`, duplicata em outra grafia, número, `null`, objeto vazio) resulta, após reload, em `customColorPalette` só com a entrada válida deduplicada, `customColorPaletteStorageAvailable===true` (JSON parseou) e `customColorInvalidStoredEntriesDropped>0`; `PROJECT_BG_NEUTRALS` continua com 11 entradas. JSON totalmente malformado resulta em `customColorPalette===[]` e `customColorPaletteStorageAvailable===false`, sem quebrar o boot do app; neutros built-in continuam de pé.
- **Persistência entre reload e entre projetos:** gravar a paleta via `localStorage` e recarregar preserva `customColorPalette`; abrir um projeto (mesmo fixture, nova instância de estado) após o reload preserva a mesma paleta e ela volta a aparecer nos três contextos (`#bgSwatches .bg-swatch-custom` com as 3 entradas).
- **Ambiente:** WebKit é o gate obrigatório (checks do HEAD final); verificação equivalente local executada em **Chromium `hasTouch`** (WebKit ausente no ambiente desta sessão) — o gate passa; suíte completa com 25 passes e as mesmas 2 falhas pré-existentes da base (404 de rede no smoke inicial; E9B multi-touch), sem regressão nova. Validação física obrigatória em iPhone/Safari por Roberto **pendente**.
