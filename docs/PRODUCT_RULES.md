...
4. **Paleta rápida compartilhada:** Cor do texto e Fundo da caixa usam sequências compactas de swatches que **reutilizam a mesma constante única** `PROJECT_BG_NEUTRALS` (preto + escala de cinza 10→90% + branco), a mesma fonte agora consumida também pelo seletor de "Cor de fundo" do PROJETO. Não existem listas de neutros hardcoded duplicadas. Cada paleta termina com um botão `+` compacto (não preenchido como cor) — `aria-label` "Escolher outra cor do texto" / "Escolher outra cor do fundo". **Regra SUPERADA pela E9F4/REG-055 (2026-08-21, ver abaixo):** a formulação original desta regra dizia que o `+` abre o color picker nativo e que "não há schema persistente novo de histórico de cores; o custom atual vive apenas em estado de UI/sessão". Isso deixou de ser verdade — o `+` agora abre o painel compartilhado de customização, e existe paleta pessoal persistente. O valor final de cada propriedade continua salvo no próprio Text Asset (ou no `bgColor` do projeto), sem mudança de schema canônico. O menu de Fundo do PROJETO **não** foi redesenhado (apenas o trigger "Personalizar" virou `+`, ver E9F4).

### Regra E9F4 — REG-055: painel compartilhado de customização de cor + paleta pessoal persistente (implementada)

Comportamento aprovado e IMPLEMENTADO na v8z4b32E9F4, superando o item 4 da Regra E9F1 acima:

1. Os botões `+` de Cor de Fundo do projeto, Cor do texto e Fundo da caixa abrem a **mesma experiência compartilhada** de customização (`#customColorPanel`, um único painel para os três contextos, não três painéis independentes), com preview da cor, campo hexadecimal editável e confirmação/aplicação explícita (✓). Um `input[type=color]` nativo isolado não é a solução — pode ser reaproveitado apenas como auxílio interno (os `input[type=color]` ocultos legados `#textCreationColor`/`#textBoxBackgroundColor` continuam existindo por compatibilidade, mas não são mais o mecanismo primário do `+`).
2. No Fundo do projeto, o antigo trigger "Personalizar" (quadrado cinza `input[type=color]` + campo hexadecimal sempre visíveis) foi **substituído** pelo botão `+`, coerente com os `+` de Cor do texto/Fundo da caixa; o restante do painel (neutros, título, descrição) foi preservado.
3. O campo hexadecimal aceita entrada com ou sem `#`, maiúsculas/minúsculas equivalentes, e normaliza para uma representação canônica única `#rrggbb` minúsculo. Entrada inválida não aplica, não persiste e não cria swatch.
4. Ao confirmar uma cor válida: ela é aplicada pelo caminho **canônico** do contexto de origem (`setBgColor`/`setTextColor`/`setTextBoxBackground`, semântica inalterada) e adicionada a uma **paleta pessoal persistente** (`customColorPalette`, `localStorage['arco_user_custom_colors_v1']`), compartilhada pelos três contextos, deduplicada por valor normalizado, sobrevivendo a reload e a trocar de projeto. Cores coincidentes com um neutro built-in de `PROJECT_BG_NEUTRALS` não geram swatch redundante.
5. A paleta pessoal **NÃO** integra o projeto (Save/Load, Session Restore, ProjectWorld, Text Asset) — é preferência local do usuário no navegador, sem sync/nuvem/conta.
6. Abrir o painel não aplica nada por si só; cancelar (×/alça) descarta sem criar swatch fantasma, sem Undo, sem autosave. `DEFAULT_PROJECT_BG` (`#3c3c3b`) não muda por causa desta regra.
