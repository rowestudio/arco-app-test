...
## Regra E9G1 — side width handles do Text Asset e correção de REG-056 (implementada, validação física pendente)

Uma primeira implementação da E9G (`v8z4b32E9G`, PR #512) chegou a ser mergeada e foi **revertida** em 2026-08-23 após a build publicada ser REPROVADA FISICAMENTE por Roberto (REG-057: reedição de Text Asset não reabre o editor após o ciclo edição → confirmação → Preview → sair do Preview). O teste físico do rollback (build restaurada `v8z4b32E9F6`) confirmou o mesmo fluxo funcionando ("OK"); REG-057 está registrada como RESOLVIDA FISICAMENTE PELO ROLLBACK, mas a causa raiz da falha específica da antiga E9G permanece NÃO comprovada (ver `docs/REGRESSIONS.md`).

A `v8z4b32E9G1` é uma nova implementação, sobre essa mesma base restaurada, com as seguintes regras aprovadas:

1. **Side width handles exclusivas de Text Asset.** Não-texto continua com exatamente quatro corner handles (`tl`/`tr`/`bl`/`br`), zero side handles. Text Asset ganha exatamente quatro corner handles **mais** exatamente duas side width handles (`.text-width-handle`, `data-asset-width-handle="left"/"right"`), num sistema próprio que nunca reaproveita `.asset-corner-handle` nem os handlers de escala/rotação. REG-028 é especializada por tipo de asset, não enfraquecida globalmente.
2. **Width ≠ scale.** Corner handles continuam responsáveis SOMENTE pela escala proporcional já existente; sua matemática não muda além do necessário para coexistir com as side handles. Side width handles alteram SOMENTE a largura lógica da caixa, o reflow decorrente e a posição do centro na medida necessária para preservar a âncora da lateral oposta — nunca `fontSize`, `textBaseFontSize`, percentual de escala, `rotation`, `depth`, `zIndex`, texto, fonte, estilo, alinhamento, cor, fundo, opacidade, padding, Frames, curvas ou timing. A razão de escala no início do gesto permanece constante durante todo o drag.
3. **REG-056 — o limite é a largura LÓGICA.** A fonte de verdade do clamp é `textBaseBoxWidth` (via `getTextWidthLogicalBounds()`), nunca `boxWidth` já escalado. A mesma faixa lógica fica disponível para o mesmo Text Asset em qualquer escala (verificado em ~50/100/200%). Não existe uma segunda definição divergente de limites.
4. **Geometria do drag.** O deslocamento do pointer é projetado no eixo horizontal LOCAL do Text Asset (nunca delta X bruto de tela), usando as transformações canônicas Stage ↔ World já existentes (`screenToStageCoord`/`editorStageToWorld`). Funciona sob rotação, inclusive ~45°. Arrastar a direita mantém a lateral esquerda ancorada no World; arrastar a esquerda mantém a lateral direita ancorada; o centro desloca somente o necessário.
5. **Auto → Fixed.** O primeiro delta EFETIVO de uma side width handle transforma o modo em `fixed` no próprio gesto. Um gesto totalmente absorvido pelo clamp, sem alteração real, não é tratado como edição efetiva (sem mudança de modo, sem Undo, sem autosave).
6. **`pendingTextDraft` continua sendo a única fonte de verdade visual** enquanto o editor está ativo. Side width drag durante a edição modifica o draft WYSIWYG imediatamente, sem Undo/autosave durante o draft. Confirmar com mudança preserva o mesmo asset ID, cria exatamente 1 Undo e 1 revisão/autosave, e preserva a âncora visual estabelecida pelo width drag (o centro de recomposição no commit de edição vem do próprio draft, não do centro pré-edição do asset confirmado).
7. **Fora do editor**, um drag completo e efetivo de side width handle produz exatamente 1 Undo + 1 autosave; um gesto no-op/clamp produz 0 Undo/0 autosave/0 dirty artificial. `pointercancel` restaura integralmente o snapshot pré-gesto (largura, centro, modo Auto/Fixed, escala, seleção, estado de gesto), sem Undo/autosave decorrente do cancelamento.
8. **Painel Largura.** O slider/stepper manual de largura (E9F1) foi removido definitivamente; o painel mantém somente a ação compacta Auto. Não existem duas formas concorrentes de edição manual de largura.
9. **Proteção do gatekeeper de seleção.** Side width handles recebem a mesma proteção específica contra o gatekeeper de seleção do Stage que já existe para corner handles (`handleStageAssetSelectPointer` exime `.asset-corner-handle,.text-width-handle`), sem enfraquecer genericamente a seleção do Stage. Este é um aprendizado herdado da primeira tentativa da E9G — **não é a causa comprovada da REG-057**, cuja causa raiz permanece desconhecida.

O código desta regra NÃO promove nenhuma reivindicação de causa raiz para a REG-057 original; apenas registra o comportamento exigido/implementado da E9G1. Ver `docs/DECISIONS.md` para o histórico completo das decisões (E9G original SUPERADA/REVERTIDA; rollback; nova tentativa E9G1) e `docs/REGRESSIONS.md` (REG-056, REG-057, REG-028) para o estado corrente.
