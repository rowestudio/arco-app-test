...
## Regra E9H — Camadas (affordance + sheet parcial) e Profundidade (régua/−10/+10) (implementada, em PR)

Comportamento aprovado e IMPLEMENTADO na `v8z4b32E9H` — readequação da interface de Layers/Camadas já existente e revisão visual do controle de Profundidade já existente. Não cria Layers nem Profundidade do zero; toda a lógica de seleção, zIndex, visibilidade e depth é a mesma já existente.

1. **Affordance compacto de Camadas.** No Modo Ativos, `#layersAffordance` (canto inferior direito do Stage) representa a Layer atualmente selecionada (thumbnail do asset, ou ícone de tipo quando não há thumbnail, ou ícone genérico sem seleção) e é a **entrada principal** da interface de Camadas. Não reutiliza o ícone existente de visualização/referências (`#stageEyeShortcut`). Não aparece fora do Modo Ativos, nem em Preview/Export/gravação, nem sem imagem carregada.
2. **Sheet parcial de Camadas.** `#layersPanel` abre a partir de baixo, com alça, título "Camadas" e `×` para fechamento explícito. Altura máxima ~40% do viewport, com rolagem vertical interna própria quando a quantidade de Layers excede a área disponível; nunca modal full-screen. O Stage permanece visível (e interativo) acima da sheet. A entrada "Layers" saiu do menu inferior de Ativos (`assetsModeContextMenu`); o affordance compacto é agora a entrada.
3. **Lista de Camadas.** Cada linha mostra, de forma compacta: thumbnail ou ícone de tipo, nome persistente (`getAssetDisplayName`, inalterado), posição na pilha (1 = topo) e profundidade atual com sinal.
4. **Seleção.** Tocar numa Layer chama a mesma `selectAssetById(id,'layers')` já existente — altera a seleção canônica do Stage, atualiza o affordance e os controles do asset — e **nunca fecha a sheet**. Não existe estado de seleção paralelo exclusivo do painel.
5. **Ações por Layer.** Visibilidade (novo `toggleAssetVisibility`, expõe o campo `asset.visible` já respeitado pelo renderer/hit-test/export — não é um segundo sistema de visibilidade), frente/trás (`layerMoveAssetUp`/`layerMoveAssetDown` existentes, alteram somente `zIndex`), trocar imagem quando aplicável (`layerReplaceImageForAsset` existente), excluir (novo `layerDeleteAsset`, delega a `deleteSelectedAsset()` existente) e Profundidade (mesmo `assetContextPanelKind==='depth'`).
6. **Régua visual de Profundidade.** Intervalo público contínuo `−100` a `+100` preservado, sem snap. A régua (`#assetContextDepthRuler`) mostra apenas três números — `−100`, `0`, `+100` — e oito ticks intermediários sem número (`−80/−60/−40/−20/+20/+40/+60/+80`); o tick de `0` é visualmente mais destacado que os demais.
7. **Preenchimento bidirecional.** O preenchimento do slider de Profundidade (`updateDepthSliderFill`, função isolada de `updateSliderFill`) parte do zero em direção ao thumb — valor negativo preenche do zero para a esquerda, positivo do zero para a direita, zero sem preenchimento direcional — usando o mesmo `var(--accent)` do workspace Ativos (coral `#FF6B8A`; Frames preserva o ciano existente, intocado). Não altera o fill unidirecional dos demais sliders (REG-052 permanece aberta e intocada).
8. **Botões rápidos `−10`/`+10` (decisão nova, substitui `−5`/`+5`).** `−10` subtrai 10 do valor corrente; `+10` soma 10; ambos respeitam o clamp `−100`/`+100`; Reset define exatamente 0 (`resetAssetContextValue`, inalterado); nenhum dos dois altera `zIndex`; o slider continua sem snap para múltiplos de 10. A formulação anterior de `−5`/`+5` para Profundidade (nunca implementada) está **SUPERADA** — ver `docs/PRE_PROMOTION_RELEASE_PLAN.md`/`docs/DECISIONS.md`.
9. **Invariantes preservados.** `depth` continua independente de `zIndex`; nenhum controle desta revisão altera silenciosamente o outro. Painel/affordance de Camadas e a revisão de Profundidade são UI de edição — não aparecem em Preview nem Export, e não alteram ProjectWorld. Um gesto confirmado de Profundidade (arraste ou botão) gera exatamente uma entrada de Undo e uma revisão de autosave; estado intermediário de arraste não cria avalanche de histórico.
10. **Fora de escopo desta regra.** Text Asset/editor de Texto, REG-056, side width handles, REG-052 (fill dos demais sliders), Vista Profundidade 0/absoluta, animação de depth, Motion Path, Groups e Engine Sprint não foram tocados nem antecipados.
