...
- **Histórico factual registrado (recuperado do log de commits, pois o revert da PR #510 removeu a narrativa documental correspondente):**
  - **PR #506 / v8z4b32E9F3 — mergeada** (REG-053: painel de transformação em multi-seleção). Não relacionada a cores.
...
- **Ativação Safari-safe:** reaproveita da #509-R2 (única parte comprovadamente útil da E9F5) a primitiva `.color-trigger-wrap` para o "+" de Cor do texto/Fundo da caixa — "+" visual `pointer-events:none` sobreposto pelo MESMO `input[type=color]` nativo (nunca clonado/recriado/remontado, mesmo durante os próprios eventos do picker), declarado uma única vez no HTML como último filho estático da linha de swatches. Sem `.click()` programático, sem input 1×1/off-screen. Fundo do projeto já usava um `input[type=color]` real e visível (nunca 1×1/off-screen, nunca `.click()`) — preservado sem alteração estrutural, só a separação de origem picker×hex no commit (`commitBgColorEdit(hex, source)`).
...
- **Testes:** dois novos gates `REG-055 —` em `tests/smoke/app.spec.mjs` (ver `docs/TEST_CASES.md`). Verificação equivalente em **Chromium `hasTouch`** (WebKit ausente no ambiente): ambos os gates passam na `v8z4b32E9F6`; o gate de runaway/HEX/recovery foi confirmado FALHANDO contra o código revertido da E9F5 (`git worktree` isolado no commit `3c64bdb`, tip da PR #509) — a paleta pessoal cresceu de 200 para 206 entradas durante a sessão simulada de arraste, reproduzindo exatamente REG-055A, e também falha por ausência do purge de v1 (REG-055C). Suíte completa `tests/smoke/app.spec.mjs` executada (28 testes): 26 passam; as 2 falhas restantes (smoke inicial: 404 de rede; E9B: gesto multi-touch) são idênticas à base pré-mudança (confirmado por execução comparativa) e já documentadas como limitações do ambiente Chromium — sem nenhuma regressão nova introduzida. `tests/smoke/export.spec.mjs` (WebKit macOS) não executado neste ambiente (WebKit ausente); os dois usos de `#textCreationColor`/`#textBoxBackgroundColor` nesse arquivo usam apenas IDs preservados nesta PR, sem alteração estrutural esperada.
- `APP_VERSION` = `APP_VERSION_NAME` = `v8z4b32E9F6`.
- **Estado:** implementação em PR no repositório de teste; execução local em WebKit bloqueada pela ausência do WebKit. Checks obrigatórios do HEAD final (QA Guardrails; WebKit Smoke Tests; WebKit functional smoke; Real Export Smoke / WebKit macOS; H.264 real; REG-055 E9F6; E9F1; REG-053; REG-054) devem pertencer ao HEAD final da PR. **Validação física obrigatória em iPhone/Safari por Roberto permanece pendente**; REG-055 só será marcada resolvida integralmente após esse teste, repetindo o protocolo físico do ticket (picker sem criar swatches nos três contextos; HEX criando exatamente um; reload preservando; contaminação antiga da E9F5 não reaparecendo; volume de cores reais não quebrando o painel). REG-052 permanece ABERTA. Produção intocada; nenhuma promoção autorizada; nenhum merge automático.
