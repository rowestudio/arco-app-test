...
- A `v8z4b32E8V` corrige exclusivamente essa continuidade no container contextual real e adota os valores canônicos: fundo principal `#3C3C3C`, superfície contextual `#434247` e controles neutros `#505054`. A revisão estrutural final da PR #484 identifica que `#lowerContextSlot` conservava a altura do conteúdo contextual e não era o shell que ocupava toda a área inferior; `#lowerContextSheetShell` passa a ser o item de grid e ancestral estrutural único, abrangendo as tracks 3–4 e a safe-area até a viewport, enquanto `#lowerContextSlot` e os painéis preservam a geometria interna e a largura E8O. Não há pintura de `body` ou timeline. Esses tokens não autorizam mudanças nos acentos funcionais ciano/roxo.
