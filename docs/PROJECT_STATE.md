...
## Atualização 2026-08-22 — v8z4b32E9F5-R2 em PR #509 (Cores — REG-055: 2ª correção de revisão, wrapper/input persistentes no DOM)

- **Base confirmada:** PR #509 atual, HEAD revisado `eca607c0579b2330dd9b0bcca495dd08815be43b` (commit da R1). `git fetch origin` executado; `rowestudio/arco-app-test` confirmado; HEAD da PR #509 e `origin/main` (`28a0b5848fc0fe3fec97ccb3c6e192c17b01d541`, sem avanço) reconfirmados; `git status` limpo; nenhum trabalho local não relacionado descartado. MESMA branch/PR/versão — nenhuma PR nova, nenhuma E9F6, nenhum merge.
- **Motivo:** revisão técnica da PR #509 apontou que a correção de ativação direta da R1 (input nativo como alvo real do toque) estava conceitualmente correta, mas introduziu uma regressão: `renderProjectBgSwatches()`/`renderTextColorSwatches()`/`renderTextBgSwatches()` reposicionavam o input a cada render via `row.innerHTML = …` + `wrap.appendChild(input)` — e `setTextColor()`/`setTextBoxBackground()` chamam essas funções de render a CADA evento `input` do PRÓPRIO picker nativo. O input ativo era, portanto, removido do DOM e reinserido enquanto o usuário ainda podia estar interagindo com o picker, risco real de interromper a interação no iPhone/Safari.
- **Correção (mesma versão `v8z4b32E9F5`, sem bump — "R2"):** `#bgColorTriggerWrap`/`#bgHexInput`, `#textColorTriggerWrap`/`#textCreationColor` e `#textBgTriggerWrap`/`#textBoxBackgroundColor` passam a ser declarados uma única vez, estaticamente, no HTML, como último filho de cada linha de swatches. As três funções de render nunca mais tocam esses elementos: removem só os swatches dinâmicos anteriores (presets/paleta pessoal/"Sem cor") e inserem os novos via `insertAdjacentHTML('beforebegin', …)` relativo ao wrapper estático. Wrapper e input mantêm identidade, conexão ao DOM e relação pai/filho estáveis durante toda a interação. Lógica de aplicação e arquitetura de ativação direta da R1 preservadas integralmente, sem nenhuma mudança.
- **Gate `REG-055` ampliado:** nova função `assertColorTriggerNeverRemounted()` usa `MutationObserver` no `childList` da linha de swatches para detectar se o wrapper/input aparecem em `removedNodes` durante a própria reação a `input`/`change` — checar só identidade de objeto (`===`) não bastaria, pois `appendChild` reaproveita o MESMO objeto DOM. Aplicada aos três contextos (Fundo do projeto, Cor do texto, Fundo da caixa), embutida nas três provas de "escolha pelo picker" (Teste D) já existentes. **Verificação retroativa:** este novo assert foi executado contra o código da R1 (commit `eca607c`, antes desta correção) e falhou corretamente (`removedDuringUpdate: true` para Fundo do projeto), confirmando que a prova anterior (`elementFromPoint`/geometria) não capturava esta classe de regressão; após a correção R2, o gate passa.
- **Project OS corrigido nesta rodada** (a documentação da R1 ainda descrevia mecanismos já removidos ou desatualizados):
  - `docs/REGRESSIONS.md`: banner-resumo corrigido — não afirma mais que REG-053 tem validação física pendente (RESOLVIDA na E9F3, ver atualização 2026-08-21 abaixo); seção da correção de REG-055 reescrita em fases (inicial → R1 → R2 → arquitetura final), removendo a descrição de `openBgColorPicker()`/`openTextColorPicker()`/`openTextBgColorPicker()`/input off-screen já eliminados pela R1.
  - `docs/PRODUCT_RULES.md`: regra canônica dos seletores de cor ganha a exigência explícita de persistência do wrapper/input no DOM; a regra histórica E9F1 de paleta é marcada como superada especificamente no ponto "não há schema persistente" (a E9F5 acrescenta exatamente essa persistência, sem reabrir REG-055 nem alterar o resto da regra E9F1); esclarecido que o `aria-label` acessível do "+" pertence ao input nativo, não ao botão decorativo.
  - `docs/DECISIONS.md`: nova decisão `DEC-2026-08-22-02` registrada, incluindo a alternativa rejeitada (mitigar sintomas em vez de eliminar a janela de desconexão do DOM).
- **Não altera:** REG-052 (permanece ABERTA, intocada), REG-054 (permanece PARCIAL, intocada), correção estrutural de REG-053 (permanece RESOLVIDA fisicamente na E9F3), Preview/Export/renderer/Engine/curvas/timing/ProjectWorld/Layers/Save/Load, produção.
- **Testes:** gate `REG-055` reexecutado na verificação equivalente em **Chromium `hasTouch`** (binário pré-instalado do ambiente; WebKit não pôde ser baixado nesta sessão — bloqueado pelo proxy de rede local) — passa após a correção R2; falha retroativamente contra o HEAD da R1, confirmando que a nova prova de estabilidade é sensível à regressão real. Suíte completa `tests/smoke/app.spec.mjs` a reexecutar no HEAD final antes do push (ver seção de testes desta atualização abaixo, se aplicável).
- `APP_VERSION` = `APP_VERSION_NAME` = `v8z4b32E9F5` (mantida, sem bump — segunda correção de revisão da mesma PR).
- **Estado:** atualização técnica sobre a MESMA PR #509 (mesma branch, mesmo título). Validação física obrigatória em iPhone/Safari por Roberto **permanece pendente**, incluindo a abertura efetiva da UI nativa (roda/espectro/conta-gotas) do Safari/iOS quando disponibilizada — o headless não simula essa UI do sistema. Produção intocada; nenhuma promoção autorizada; nenhum merge realizado.

## Atualização 2026-08-22 — v8z4b32E9F5-R1 em PR #509 (Cores — REG-055: correção de revisão, ativação direta do input nativo)

- **Base confirmada:** PR #509 atual, HEAD revisado `5aa247d59d89d7a6526e24d97350cf502aab22c9`. `git fetch origin` executado; `rowestudio/arco-app-test` confirmado; HEAD da PR #509 e `origin/main` (`28a0b5848fc0fe3fec97ccb3c6e192c17b01d541`, sem avanço) reconfirmados; `git status` limpo; nenhum trabalho local não relacionado descartado. MESMA branch/PR/versão — nenhuma PR nova, nenhuma E9F6, nenhum merge.
- **Motivo:** revisão técnica da PR #509 apontou que a REG-055 física original (toque no "+" de Cor do texto/Fundo da caixa não abria o picker nativo no iPhone/Safari) **não havia sido efetivamente corrigida**: a primeira rodada da E9F5 preservou `#bgHexInput`/`#textCreationColor`/`#textBoxBackgroundColor` como inputs 1×1 px, deslocados para fora da tela (`left:-9999px`) e `pointer-events:none`, abertos só por um `input.click()` programático no `onclick` do "+" — exatamente o mecanismo de ativação já em vigor quando Roberto reproduziu a falha física. Provar que `.click()` foi chamado não prova que o toque do usuário chega ao picker nativo.
- **Correção (mesma versão `v8z4b32E9F5`, sem bump — "R1"):** o próprio `input[type=color]` nativo de cada contexto (nunca clonado, nunca recriado) passa a ser fisicamente REALOCADO a cada render sobre a área inteira do "+" via a primitiva `.color-trigger-wrap` (`position:absolute; inset:0; pointer-events:auto`), com o botão "+" visual por baixo em `pointer-events:none`. O input torna-se o alvo real do toque, comprovável por `document.elementFromPoint()` no centro do "+". As funções `openBgColorPicker()`/`openTextColorPicker()`/`openTextBgColorPicker()` (o `.click()` programático que era a origem do problema) foram **removidas**, não mantidas como indireção morta. Lógica de aplicação (`setBgColorHex`/`commitBgColorEdit`, `setTextColor`, `setTextBoxBackground`, paleta pessoal, HEX inline, Undo/dirty, Sem cor/opacidade) preservada integralmente, sem nenhuma mudança.
- **Gate `REG-055` reescrito:** a versão anterior só espionava se `input.click()` fora chamado — não provava que o toque chegava ao input. O novo gate prova estruturalmente, nos três contextos: bounding rect do input == bounding rect do "+" (não 1×1, não fora da tela), `pointer-events` computado != `none`, e `document.elementFromPoint()` no centro do "+" resolve para o próprio input (após `scrollIntoViewIfNeeded()`, já que as linhas de swatches rolam horizontalmente e o "+" é o último item). Também passou a exercitar `input`/`change` reais no `input[type=color]` de CADA contexto (não só HEX) e adicionou cobertura de HEX inválido em Fundo da caixa (antes só HEX válido era testado ali).
- **Project OS corrigido nesta rodada** (inconsistência documental criada após o revert da #508, sem relação com código):
  - `docs/REGRESSIONS.md`: **REG-053 agora corretamente marcada RESOLVIDA fisicamente na `v8z4b32E9F3`** (Roberto validou em iPhone/Safari) — a formulação anterior dizia erroneamente "validação física pendente" para REG-053 nesta PR.
  - `docs/ROADMAP.md`: as quatro regressões (REG-052/053/054/055) passam a ser listadas **individualmente** com status próprio, em vez de agrupadas sob uma única frase de "validação física pendente" que não se aplicava a todas.
  - `docs/DECISIONS.md`/`docs/PRODUCT_RULES.md`: removida a formulação de que o padrão `input.click()` sobre input off-screen era "já aprovado" — REG-055 é evidência física do contrário. Nova decisão `DEC-2026-08-22-01` registrada; regra canônica atualizada para "ativação DIRETA e Safari-safe do input nativo".
- **Não altera:** REG-052 (permanece ABERTA, intocada), matemática/targets de REG-054 (permanece PARCIAL, intocada), correção estrutural de REG-053, Preview/Export/renderer/Engine/curvas/timing/ProjectWorld/Layers/Save/Load, produção.
- **Testes:** gate `REG-055` reescrito passa na verificação equivalente em **Chromium `hasTouch`** (binário pré-instalado do ambiente; WebKit não pôde ser baixado nesta sessão — bloqueado pelo proxy de rede local). Suíte completa `tests/smoke/app.spec.mjs` reexecutada após a correção: mesmas 25/27 passando de antes (as 2 falhas restantes — 404 de rede no smoke inicial; gesto multi-touch do E9B — são idênticas à base e limitações conhecidas do ambiente Chromium, não regressões desta correção), incluindo E9F1/REG-053/REG-054 sem regressão nova.
- `APP_VERSION` = `APP_VERSION_NAME` = `v8z4b32E9F5` (mantida, sem bump — correção de revisão da mesma PR).
- **Estado:** atualização técnica sobre a MESMA PR #509 (mesma branch, mesmo título). Validação física obrigatória em iPhone/Safari por Roberto **permanece pendente**, incluindo a abertura efetiva da UI nativa (roda/espectro/conta-gotas) do Safari/iOS quando disponibilizada — o headless não simula essa UI do sistema. Produção intocada; nenhuma promoção autorizada; nenhum merge realizado.

## Atualização 2026-08-21 — v8z4b32E9F5 em PR (Cores — REG-055: picker completo preservado, HEX inline e paleta pessoal persistente)

- **Base confirmada:** `origin/main` do repo de teste no merge da **PR #508 / revert da E9F4**, commit **`28a0b5848fc0fe3fec97ccb3c6e192c17b01d541`** (tree `e382bfb639361b8dd866d5fcbed27ab9e09916b3`, idêntico ao tree funcional `v8z4b32E9F3` anterior à E9F4). `git fetch origin` executado; `rowestudio/arco-app-test` confirmado; `git status` limpo; **nenhuma PR aberta concorrente** identificada; nenhum trabalho local não relacionado descartado. Branch de trabalho própria; nenhum merge/promoção.
- **Histórico obrigatório registrado nesta atualização** (a `main` estava desatualizada após o revert da #508 não ter recriado a documentação da rejeição):
  - **E9F3 / PR #506:** mergeada; Roberto testou fisicamente em iPhone/Safari e **aprovou visualmente**; REG-053 teve a correção validada fisicamente nesse teste.
  - **E9F4 / PR #507:** implementou um **novo painel próprio do Arco** (`#customColorPanel`), moveu a entrada hexadecimal para dentro dele e fez os três botões "+" (Fundo do projeto, Cor do texto, Fundo da caixa) abrirem esse painel em vez do picker nativo existente. Foi **mergeada** e **Roberto testou fisicamente e REPROVOU**: a intenção nunca foi substituir o seletor completo existente nem criar um segundo painel de customização.
  - **PR #508:** reverteu **integralmente** a #507, restaurando byte a byte o tree funcional E9F3. `main` retornou a esse estado antes desta PR.
  - **REG-055** continuava **ABERTA** na `main` pré-E9F5 (o revert da #508 removeu também a correção da E9F4, deixando o sintoma original — "+" sem abrir o picker nativo — sem solução válida).
- **Escopo (correção funcional/UX de REG-055; risco médio — interface de cores, Text Asset, background do projeto, persistência browser-local):** implementa a intenção de produto definitivamente esclarecida por Roberto em 2026-08-21 (ver `docs/REGRESSIONS.md`, REG-055): os "+" abrem o picker completo já existente (sem roda/espectro/conta-gotas próprios); o HEX é INLINE no painel onde o usuário já está; nenhum painel/modal/sheet novo é criado pelo Arco; cores customizadas escolhidas pelo picker ou por HEX entram numa paleta pessoal persistente, browser-local e compartilhada pelos três contextos, fora do projeto.
- **Preservação explícita da LÓGICA DE APLICAÇÃO dos três fluxos existentes:** `beginBgColorEdit`/`commitBgColorEdit`/`setBgColor`/`setBgColorHex`/`onBgHexText` (Fundo do projeto), `setTextColor`/`#textCreationColor` (Cor do texto) e `setTextBoxBackground`/`#textBoxBackgroundColor` (Fundo da caixa) continuam intactos; nenhum foi substituído. **NOTA (corrigida na atualização 2026-08-22 acima, `v8z4b32E9F5-R1`):** o mecanismo de ATIVAÇÃO descrito originalmente aqui (`openBgColorPicker()`/`openTextColorPicker()`/`openTextBgColorPicker()` chamando `.click()` num input 1×1/off-screen/`pointer-events:none` oculto por `.text-swatch-custom-input`) reproduzia exatamente o mecanismo já responsável pela falha física de REG-055 e foi substituído por ativação direta — ver entrada 2026-08-22.
- **HEX inline:** acrescentado nos painéis de Cor do texto (`#textColorHexText`) e Fundo da caixa (`#textBoxBackgroundHexText`); o Fundo do projeto já tinha `#bgHexText`, preservado. Validador único `normalizeColorHex` (aceita com/sem "#", maiúsculas/minúsculas, sem alpha) reutilizado nos três contextos; `normalizeBgHex` passa a delegar para ele.
- **Paleta pessoal persistente:** `customColorPalette`, `localStorage` (`arco_user_custom_colors_v1`), normalização/dedup/descarte defensivo de entradas inválidas ou storage indisponível. Compartilhada pelos três contextos; uma cor só entra por escolha efetiva (picker `change` ou HEX válido aplicado); nunca lida por `buildProjectData`/Save/Load/Session Restore/Text Asset — confirmado por auditoria do código e pelo gate automatizado (TESTE J).
- **Não altera:** REG-052 (fill dos sliders, permanece ABERTA), matemática/targets de REG-054, correção estrutural de REG-053, `boxBackgroundEnabled`/regra de opacidade do Fundo da caixa, Undo/Redo, Preview/Export/renderer/`getStateAtT`/`drawAtT`, curvas/easing/timing, ProjectWorld, Save/Load, Layers, Stage, timeline, produção.
- **Diagnóstico observacional novo** (via `buildDiagnosticsText()`, sem texto técnico na UI): `customColorPaletteStorageAvailable`, `customColorPaletteCount`, `customColorDuplicatePrevented`, `customColorInvalidStoredEntriesDropped`, `lastCustomColorSource`, `lastCustomColorContext`. Nenhum diagnóstico de `customColorPanel` foi criado (o painel não deve existir).
- **Testes:** novo gate `REG-055 —` em `tests/smoke/app.spec.mjs` (ver `docs/TEST_CASES.md`), usando o fluxo público real (mesmos elementos tocados pelo usuário) nos três painéis, com assert explícito de ausência de `#customColorPanel`. Verificação equivalente executada em **Chromium `hasTouch`** (binário pré-instalado do ambiente; WebKit não pôde ser baixado nesta sessão — bloqueado pelo proxy de rede do ambiente): suíte completa de `tests/smoke/app.spec.mjs` executada, incluindo E9F1, REG-053 e REG-054, sem nenhuma regressão nova introduzida pela E9F5. Os checks obrigatórios do HEAD final (QA Guardrails, WebKit Smoke Tests, Real Export Smoke/WebKit macOS, H.264 real, gate E9F5/REG-055, E9F1, REG-053, REG-054) devem pertencer ao mesmo HEAD SHA da PR.
- `APP_VERSION` = `APP_VERSION_NAME` = `v8z4b32E9F5`.
- **Estado:** implementação em PR no repositório de teste; execução local em WebKit bloqueada pela indisponibilidade de download do binário nesta sessão (verificação equivalente em Chromium `hasTouch`). **Validação física obrigatória em iPhone/Safari por Roberto permanece pendente**, incluindo roda/espectro e conta-gotas do picker nativo quando disponibilizados pelo Safari/iOS; REG-055 só será marcada resolvida integralmente após esse teste físico. REG-052 permanece ABERTA. Produção intocada; nenhuma promoção autorizada.
