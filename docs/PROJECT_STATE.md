...
## Atualização 2026-08-03 — v8z4b32E8M em desenvolvimento

- Base confirmada na `main` no HEAD obrigatório `27eeb1318f6a4205704a7a86e2baa14f4b3cac7b`, versão `v8z4b32E8J`.
- A inserção abre diretamente a Fototeca/seletor, prepara JPEG, PNG e WebP integralmente e entra em modo modal exclusivo no Stage, com uma imagem provisória por vez, quatro abas e gestos de mover, escalar e rotacionar.
- Cada `OK` preserva somente a geometria provisória; o último confirma o lote inteiro atomicamente, com ordem original, um Undo e uma revisão de Session Autosave. `Cancelar` descarta o lote e restaura a seleção e os estados transacionais anteriores.
- O modal de organização automática e as opções Empilhar, Horizontal e Vertical foram removidos do fluxo.
- Preview, Export, WebCodecs, renderer, Frames, curvas, timeline, Save/Load e Session Restore permanecem preservados fora do diff funcional específico.
- Validação real em iPhone/Safari permanece pendente. Nenhuma promoção para produção está autorizada.
