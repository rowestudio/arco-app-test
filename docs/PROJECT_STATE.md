...
## Atualização 2026-08-18 — v8z4b32E9E em PR (Texto — estabilização funcional)
...
- Estado: implementação em PR no repositório de teste; execução local em WebKit permanece bloqueada pela ausência do WebKit (verificação equivalente feita em Chromium `hasTouch`). Validação visual final em iPhone/Safari real por Roberto permanece obrigatória; produção não foi tocada e nenhuma promoção está autorizada.
