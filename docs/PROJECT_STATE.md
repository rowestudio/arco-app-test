...
- A `v8z4b32E8V` corrige exclusivamente essa continuidade no container contextual real e adota os valores canônicos: fundo principal `#3C3C3C`, superfície contextual `#434247` e controles neutros `#505054`. A revisão estrutural da PR #484 remove a compensação fixa inferior: no estado aberto, a última track do grid absorve naturalmente o espaço restante e a safe-area dentro do próprio `#lowerContextSlot`, cujo retângulo termina na borda da viewport sem pintar `body` ou timeline. Esses tokens não autorizam mudanças nos acentos funcionais ciano/roxo.
