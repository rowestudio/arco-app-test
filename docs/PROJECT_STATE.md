...
- **Preservação explícita dos três fluxos existentes:** `beginBgColorEdit`/`commitBgColorEdit`/`setBgColor`/`setBgColorHex`/`onBgHexText` (Fundo do projeto), `openTextColorPicker()`/`#textCreationColor` (Cor do texto) e `openTextBgColorPicker()`/`#textBoxBackgroundColor` (Fundo da caixa) continuam intactos como caminho canônico; nenhum foi substituído. Novo `openBgColorPicker()` para o Fundo do projeto segue exatamente o mesmo padrão já aprovado (semeia o valor atual e chama `.click()` no `input[type=color]` já existente, agora visualmente oculto pelo mesmo mecanismo `.text-swatch-custom-input` das outras duas paletas, com um botão "+" (`.bg-swatch-add`) coerente com as demais).
