...
- Regressões abertas recém-relatadas devem ser consideradas antes de avançar a próxima frente. Está ABERTA no momento: **REG-052** (fill dos sliders). **REG-053** (painel de transformação cortado em multi-seleção) foi **validada fisicamente por Roberto e está RESOLVIDA** (`v8z4b32E9F3`, PR #506). **REG-054** (transformação de multi-seleção afeta apenas um Frame) teve correção implementada/automatizada (`v8z4b32E9F2`), com validação física **parcial** pendente de conclusão. **REG-055** — a interpretação original ("botões `+` dos color pickers do Text Asset") foi clarificada por Roberto (2026-08-21): `+` significa adicionar/personalizar cor com paleta pessoal persistente compartilhada; correção/aprimoramento implementado/automatizado na `v8z4b32E9F4`, com validação física pendente. Ver `docs/REGRESSIONS.md` para o detalhe de cada uma. Este roadmap não altera automaticamente a ordem por causa delas: **Roberto decide a ordem efetiva** entre validar/fechar essas regressões e avançar a série pré-promoção.
