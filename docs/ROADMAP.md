...
1. PR funcional de Texto (correção de regressão + melhoria visual localizada), subdividida na rodada de Texto:
   ...
   - **E9G — manipulação direta da largura da caixa no Stage por alças laterais: FUTURA, NÃO implementada.** Uma primeira tentativa (`v8z4b32E9G`, PR #512) chegou a ser mergeada e depois **REVERTIDA (2026-08-23)** após a build publicada ser REPROVADA FISICAMENTE por Roberto (REG-057: reedição de Text Asset não reabre o editor após o ciclo edição → confirmação → Preview). Ver `docs/DECISIONS.md` (DEC-2026-08-23-02) e `docs/REGRESSIONS.md` (REG-056, REG-057). O conceito permanece aprovado como item futuro; uma nova tentativa exige revisão explícita de REG-028/exatamente quatro alças e nova validação física completa, incluindo a sequência de reedição pós-Preview (ver `docs/TEST_CASES.md`, TC-054).
