...
- Regressões abertas recém-relatadas devem ser consideradas antes de avançar a próxima frente. Está ABERTA no momento: **REG-052** (fill dos sliders). **REG-053** (painel de transformação cortado em multi-seleção), **REG-054** (transformação de multi-seleção afeta apenas um Frame) e **REG-055** (picker de cor do Arco — nova tentativa `v8z4b32E9F6` após duas reprovações físicas anteriores, E9F4/painel próprio e E9F5/runaway palette) tiveram correção implementada/automatizada (`v8z4b32E9F3`, `v8z4b32E9F2` e `v8z4b32E9F6`, respectivamente), com validação física pendente — ver `docs/REGRESSIONS.md`. Este roadmap não altera automaticamente a ordem por causa delas: **Roberto decide a ordem efetiva** entre validar/fechar essas regressões e avançar a série pré-promoção. Não avançar automaticamente Layers/Profundidade ou Engine Sprint por causa da E9F6.
