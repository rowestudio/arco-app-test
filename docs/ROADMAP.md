...
1. PR funcional de Texto (correção de regressão + melhoria visual localizada), subdividida na rodada de Texto:
   ...
   - **E9G1 — manipulação direta da largura da caixa no Stage por alças laterais: IMPLEMENTADA em PR (`v8z4b32E9G1`, 2026-08-24), validação física PENDENTE.** Uma primeira tentativa (`v8z4b32E9G`, PR #512) chegou a ser mergeada e depois **REVERTIDA (2026-08-23)** após a build publicada ser REPROVADA FISICAMENTE por Roberto (REG-057: reedição de Text Asset não reabre o editor após o ciclo edição → confirmação → Preview). O teste físico do rollback (build `v8z4b32E9F6`) confirmou o fluxo de reedição funcionando, resolvendo fisicamente a REG-057 nessa base (causa raiz da falha original permanece não comprovada). A `v8z4b32E9G1` é a nova tentativa, sobre essa mesma base: duas side width handles exclusivas de Text Asset, clamp sobre `textBaseBoxWidth` (REG-056 corrigida na origem), REG-028 especializada (não enfraquecida). Ver `docs/DECISIONS.md` e `docs/REGRESSIONS.md` (REG-056, REG-057). Exige validação física completa por Roberto, incluindo a sequência de reedição pós-Preview e o ciclo adicional com side width drag (ver `docs/TEST_CASES.md`, TC-054) antes de ser considerada resolvida fisicamente.
...
- Regressões abertas recém-relatadas devem ser consideradas antes de avançar a próxima frente. Seis regressões, seis status distintos (ver detalhe completo em `docs/REGRESSIONS.md`, não agrupar): **REG-052** (fill dos sliders) está **ABERTA**, não corrigida. **REG-053** (painel de transformação cortado em multi-seleção) foi **RESOLVIDA FISICAMENTE** — Roberto testou a build publicada da `v8z4b32E9F3` em iPhone/Safari e aprovou visualmente. **REG-054** (transformação de multi-seleção afeta apenas um Frame) teve correção implementada/automatizada na `v8z4b32E9F2`, com validação física **PARCIAL** (só a Rotação em multi-seleção confirmada fisicamente; demais itens do checklist ainda pendentes) — não tratar como totalmente validada. **REG-055** (picker de cor do Arco — nova tentativa `v8z4b32E9F6` após duas reprovações físicas anteriores, E9F4/painel próprio e E9F5/runaway palette) foi **RESOLVIDA FISICAMENTE** — Roberto testou a build publicada em iPhone/Safari e reportou "Tudo ok" para o protocolo físico completo; este fato é posterior ao merge da E9F6 e é preservado pelo rollback da E9G, não voltando para pendente. **REG-056** (largura manual do Text Asset dependente da escala visual) tem nova implementação em PR (`v8z4b32E9G1`) sobre `textBaseBoxWidth`; status **IMPLEMENTAÇÃO EM VALIDAÇÃO**, não fisicamente resolvida até o novo teste de Roberto. **REG-057** (reedição de Text Asset não reabre o editor após o ciclo edição → confirmação → Preview → sair do Preview) está **RESOLVIDA FISICAMENTE PELO ROLLBACK/RESTAURAÇÃO DA E9F6** — Roberto testou esse fluxo na build restaurada e confirmou OK; causa raiz da falha original ainda não comprovada; a nova implementação E9G1 exige seu próprio teste físico completo. Este roadmap não altera automaticamente a ordem por causa delas: **Roberto decide a ordem efetiva** entre validar/fechar essas regressões e avançar a série pré-promoção. Nenhuma frente seguinte (Layers/Profundidade, Engine Sprint) foi autorizada a avançar por causa desta PR.
