...
- Regressões abertas recém-relatadas devem ser consideradas antes de avançar a próxima frente. Está ABERTA no momento: **REG-052** (fill dos sliders). **REG-053** (painel de transformação cortado em multi-seleção), **REG-054** (transformação de multi-seleção afeta apenas um Frame) e **REG-055** (picker completo/HEX inline/paleta pessoal dos seletores de cor) tiveram correção implementada/automatizada (`v8z4b32E9F3`, `v8z4b32E9F2` e `v8z4b32E9F5`, respectivamente), com validação física pendente — ver `docs/REGRESSIONS.md`. A tentativa intermediária `v8z4b32E9F4`/PR #507 de REG-055 foi mergeada, reprovada fisicamente por Roberto e revertida integralmente pela PR #508; a `v8z4b32E9F5` é a correção corretiva. Este roadmap não altera automaticamente a ordem por causa delas: **Roberto decide a ordem efetiva** entre validar/fechar essas regressões e avançar a série pré-promoção.
