...
- [ ] **Step 2: Verify failure.** Run `npx playwright test tests/smoke/app.spec.mjs --project=webkit --grep "E9BG: Layers multiselection|E9BG: group layer reorder"`. Expected: missing class/function.
...
- [ ] **Step 4: Verify group and single-row order behavior.** Run `npx playwright test tests/smoke/app.spec.mjs --project=webkit --grep "E9BG: Layers multiselection|E9BG: group layer reorder|layer.*reorder"`. Expected: PASS and existing single-row drag remains valid.
...
- [ ] **Step 2: Verify failure.** Run `npx playwright test tests/smoke/app.spec.mjs --project=webkit --grep "E9BG: group move|E9BG: group scale|E9BG: group rotation"`. Expected: current overlay has only one asset.
...
- [ ] **Step 4: Verify transforms and Undo/Redo.** Run `npx playwright test tests/smoke/app.spec.mjs --project=webkit --grep "E9BG: group (move|scale|rotation)|Undo|Redo|asset.*transform"`. Expected: PASS; single-asset tabs remain unchanged.
...
- [ ] **Step 2: Verify failure.** Run `npx playwright test tests/smoke/app.spec.mjs --project=webkit --grep "E9BG: contextual group|E9BG: group front|E9BG: group back"`. Expected: current handlers target only `selectedAssetId`.
...
- [ ] **Step 4: Verify boundaries.** Run `npx playwright test tests/smoke/app.spec.mjs --project=webkit --grep "E9BG: contextual group|E9BG: group (front|back)|opacity|depth|timing"`. Expected: PASS and none of the excluded properties change.
...
- [ ] **Step 2: Verify failure.** Run `npx playwright test tests/smoke/app.spec.mjs --project=webkit --grep "E9BG: transparent image hit test"`. Expected: top image wins everywhere inside its rectangle.
...
- [ ] **Step 4: Verify hit-test and rendering regressions.** Run `npx playwright test tests/smoke/app.spec.mjs --project=webkit --grep "E9BG: transparent image hit test|hit.?test|replace|Preview|Export"`. Expected: PASS.
...
- [ ] **Step 2: Verify failure.** Run `npx playwright test tests/smoke/app.spec.mjs --project=webkit --grep "E9BG: asset group (save load|session restore)|E9BG: selection excluded from render"`. Expected: cleanup/diagnostics assertions fail before implementation.
...
- [ ] **Step 4: Run full automated verification.** Run `npx playwright test tests/smoke/app.spec.mjs --project=webkit`, `node scripts/qa/check-project-os.mjs`, `node scripts/qa/run-self-tests.mjs`, and `git diff --check origin/main...HEAD`. Expected: all pass with no technical text rendered in the app.
