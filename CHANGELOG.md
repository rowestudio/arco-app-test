...
**Preview asset parity + visual render audit + frame curve anchor fix — Engine Sprint autorizado pela tarefa: corrige os três problemas bloqueadores remanescentes da E6 (Preview do img-1 usando source/worldRect antigo; imagem 6 sumindo/reaparecendo no Preview e no MP4; âncora da curva deslocada do centro visual do frame no Modo Câmera).** Base: `v8z4b32E6`.

- **BLOCO A — Preview do img-1 usa o MESMO caminho canônico do Export.** Diagnóstico: Preview e Export já passam ambos por `drawAtTDirect`→`drawWorldToCanvas`→`collectWorldRenderAssets` (lendo `assets[].sourceW/H` e `worldX/Y/W/H`); o Preview reportava a caixa/nat antigos porque `renderTransform.preview` e o snapshot de sessão ficavam **stale** (amostra de um Preview anterior ao replace). Correção: `_invalidatePreviewRenderCacheAfterReplaceE7()` zera `renderTransform.preview/export/world` e reconstrói o snapshot canônico no commit do replace; o Preview passa a reportar `natW/H` = `sourceW/H` e `worldRect` do asset (idênticos ao Export). Novos campos `img1Preview*` medem a amostra REAL do renderer, nunca `imgNat` global/main image legado/DOM wrapper/rect antigo.
- **BLOCO B — presença VISUAL/pixel real da imagem 6 (`img-1781638924641-484`).** O diagnóstico da E6 contava draw call/`considered`/`drawn`, mas não media presença visual. Correção: (1) **fonte estável e imutável** por asset visível — `buildRenderSessionSnapshotE6()` cria um stable drawable persistente (`HTMLCanvasElement` — seguro em iPhone/Safari — snapshot do bitmap já decodificado) por asset visível e o congela na sessão via `sourceVersion`; `collectWorldRenderAssets()` prefere essa fonte estável, então onload/decode tardio ou replace de OUTRO asset não trocam a fonte da render session ativa; (2) **auditoria visual/pixel** (`_pixelAuditKnownAssetE7`) — após o draw, projeta o `worldRect` do asset para a tela, amostra 9 pontos internos (grade 3×3 encolhida 22% para evitar bordas/transparências), lê os pixels do canvas final com `getImageData` e conta presença (alpha>8). Detecta frames com draw call mas sem pixels, queda de presença e desaparecer/reaparecer. Limitada e envolta em try — nunca altera o render. Novos campos `knownProblemAsset*Pixel*/*Visual*/*StableDrawable*`.
- **BLOCO C — âncora da curva coincide com o centro visual do frame.** Diagnóstico: no Modo Câmera multi-imagem (`_editorTransform` ativo, `baseStageW`≠`stageW`) os retângulos `.frame` usam `editorWorldToStage` (coords de tela) mas os handles IN/OUT e seus braços vinham de `getFrameHandleGeometry` em coords de MUNDO (`frameCX/frameCY`, `cp.nx*stageW`) — divergência visível como braço/handle nascendo fora do centro do frame (a curva e os dots centrais já usavam `getFilmFrameGeometry`/tela). Correção: a geometria VISUAL dos handles IN/OUT (braços em `drawBezier`, dots DOM e ghosts em `updateCtrlPts`) é convertida mundo→tela via `editorWorldToStage`; a entrada do DRAG é convertida tela→mundo via `editorStageToWorld` antes de `applyFrameConnectedHandleEdit` (que opera em `frameCX` mundo), tornando visual + drag consistentes com o retângulo e a curva. **NO-OP em projeto de imagem única** (transforms são identidade quando `_editorTransform` inativo — zero regressão). `getFrameScreenGeometry` é a fonte única de geometria de frame; `_recordFrameCurveGeometryParityE7()` mede o delta real centro-do-frame × âncora-da-curva (`frameCurveAnchorDeltaPx ≤ 1px`). Novos campos `frameCurve*`.
- **diagnóstico legado atualizado.** `computeWorldCoordinateAudit()` deixa de exigir `asset0.worldW/H == baseStage`: após o unified stack + replace canônico, img-1 é asset comum e pode ter AR própria (ex.: 341×255.8). `asset0MatchesWorldCoordinateSystem` passa a validar pertencimento ao espaço ProjectWorld (coords finitas/positivas + `worldAR≈sourceAR`), então `mixedWorldScaleDetected=false` para img-1 com AR própria; `projectWorld.w/h` continua ancorado em `baseStage`.
- **NÃO TOCADO:** `getStateAtT@v8z4b32E3`/`getStateAtTBase`/sampler canônico/paridade Preview-Export, matemática da câmera em `drawWorldToCanvas`, interpolação/curvas salvas/rotação/easing, frames salvos, timeline/duração, `projectWorld.baseStageW/H`/geometry, WebCodecs/muxer/encoder, UI/layout/cores/ícones/texto/menus/toolbar/painel Layers, "Não limitar" (ausente), comportamento aprovado de assets/move/reorder, commit canônico do img-1 da E6.
- **diagnóstico (campos observacionais protegidos).** Novos: BLOCO A `img1Preview{PathParityFixEnabled,UsesSameAssetRendererAsExport,UsesCanonicalAssetModel,UsesRenderSessionSnapshot,NatMatchesAssetSource,WorldRectMatchesAssetWorldRect,DoesNotUseLegacyMainImage,DoesNotUseGlobalImgNat,DoesNotUseOldPreviewRect,CacheInvalidatedAfterReplace,SnapshotRebuiltAfterReplace,RenderRect,RenderNat}`, `img1AssetCanonical{WorldRect,SourceWH}`; BLOCO B `knownProblemAsset{VisualPresenceChecked,{Preview,Export}PixelPresenceChecked,{Preview,Export}VisiblePixelFrames,{Preview,Export}ExpectedPixelFrames,{Preview,Export}TransparentFrameCount,{Preview,Export}VisualDropDetected,{Preview,Export}DisappearedThenReappearedVisual,DrawCallButNoPixelsDetected{Preview,Export},BitmapBecameTransparentDuring{Preview,Export},SourceBitmapStableDuring{Preview,Export},StableDrawable{Created,UsedInPreview,UsedInExport},SourceVersionStableDuring{Preview,Export},RenderSourceChangedFrameRanges,VisualMissingFrameRanges}`, `renderSession{StableDrawablesCreatedCount,AllVisibleAssetsHaveStableDrawable,DidNotStartWithoutStableSource}`; BLOCO C `frameCurveGeometryParityFixEnabled`, `frameVisualCenterScreen{X,Y}`, `curveAnchorScreen{X,Y}`, `frameCurveAnchor{DeltaPx,ParityOk}`, `activeFrame{VisualRect,Curve}UsesCanonicalGeometry`, `curve{Handles,Path}UsesCanonicalFrameCenter`, `frameHudUsesCanonicalGeometry`, `scrimCutoutUsesCanonicalGeometry`, `frameCurveGeometryCacheInvalidatedAfter{PanZoom,FrameTransform,ModeSwitch,TimelineChange}`, `duplicateFrameCenterCalculationDetected`, `staleCurveAnchorDetected`, `curveAnchorUses{UnrotatedFrameCenter,OldPanZoom,OldFrameIndex,HudClampedPoint,VisualFrameCenterNotModelCenter}Detected`, `frameCurveParityChecked{InCameraMode,WithRotation,AfterPanZoom}`, `curveHandle{VisualUsesScreenGeometry,DragUsesWorldGeometry}`. Todos protegidos.
- `index.html`: comentário do topo, `APP_VERSION` e `APP_VERSION_NAME` atualizados para `v8z4b32E7`.
