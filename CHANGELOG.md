...
- Corrige a falha real de geração de MP4 no formato **1:1** (`exportSuccess: false`, `lastExportError: "Encoding task failed"`, `lastExportOriginalErrorName: "EncodingError"`), diagnosticada na v8z4b30G. 9:16 e 4:3 continuavam funcionando; só 1:1 falhava, inclusive em arquivo novo.
- Causa raiz: 1080×1080 (1:1) equivale a 4624 macroblocos de 16×16, acima do orçamento de 3600 MBs dos níveis H.264 3.0/3.1 usados pelos candidatos de codec fixos (`avc1.42001f`/`avc1.42E01E`/`avc1.4D401F`). `VideoEncoder.isConfigSupported()` reportava suporte (`supported: true`) para esses candidatos mesmo em 1080×1080, mas o encoder real do Safari/WebCodecs falhava com `EncodingError` ao configurar/codificar. 9:16 (3600 MBs) e 4:3 (3468 MBs) ficam dentro do orçamento do nível 3.1 e por isso funcionavam.
- `index.html` (`startRecord` → novo `runWebCodecsExport()`): calcula, por resolução de export, o total de macroblocos (`ceil(W/16)*ceil(H/16)`) e, quando excede 3600 (hoje, só o 1:1), monta candidatos de codec com o nível H.264 mínimo necessário (`avc1.4D40xx`/`avc1.6400xx`/`avc1.4200xx`, nível calculado por tabela de macroblocos máximos por nível: 3.2/4.0/4.1/4.2/5.0), tentados antes dos candidatos originais. Para 9:16/4:3 (≤3600 MBs) os candidatos e a ordem permanecem **exatamente** os mesmos de antes — nenhuma mudança de comportamento.
- feat(export): novo **fallback automático e seguro** — se o caminho WebCodecs falhar em qualquer etapa (codec não suportado, `configure`, `encode`, `flush`) com qualquer erro (não só 1:1), `startRecord()` registra o erro original completo, libera o encoder e o canvas de export, e tenta o pipeline `MediaRecorder` antes de reportar falha ao usuário. 9:16/4:3 continuam usando WebCodecs normalmente; o fallback só entra em ação se o WebCodecs efetivamente falhar.
- `index.html` (`arcoExportDiag`): novos campos somente-leitura `encoderPathUsed` (`'webcodecs'` | `'fallback'`), `fallbackUsed` (booleano) e `lastCleanupStep`, exibidos no painel Diagnóstico.
- `index.html` (nova `arcoTouchCleanupStep()`): as etapas de limpeza/restauração de UI pós-export (`cleanup-start`/`cleanup-success`/`cleanup-failed`, em `finishExport` e `cleanupFailedExport`) passam a ser registradas em `lastCleanupStep`, sem mais sobrescrever `lastExportStepDetailed`/`lastSuccessfulExportStep` — corrige o diagnóstico enganoso em que `lastExportStepDetailed` mostrava `cleanup-success` mesmo com `exportSuccess: false`.
- `index.html`: versionamento atualizado para `v8z4b30H` em comentário do topo, `APP_VERSION`, `APP_VERSION_NAME` e texto visível do app.
- Preserve: nenhuma segunda imagem é renderizada/implementada; Stage, `layoutStage()`, zoom/pan/clamps, curvas, layout aprovado, `projectWorld`/`assets`/`views`/permissões, visuais/ícones/textos/menus/fluxo permanecem inalterados; sem `prompt()`/`alert()`/`confirm()`. Marca d'água/aviso premium para `>=4` frames preservados. Painéis Formato/Tempo/Movimento/Trajetória já fecham antes do Preview/export (`closeAll()`, herdado da v8z4b30F).

# v8z4b30G

- Diagnóstico/instrumentação: a v8z4b30F não resolveu o travamento da geração de MP4 no formato **1:1** — a geração trava mesmo em arquivo novo, sem depender de projeto antigo ou troca de formato. Esta versão NÃO tenta corrigir a causa raiz; instrumenta profundamente o pipeline de export para que o painel Diagnóstico diga **exatamente em qual etapa o export trava**.
- `index.html` (`arcoExportDiag`): novos campos somente-leitura — `exportSessionId` (identifica cada tentativa, evita que callbacks tardios de uma tentativa anterior corrompam o diagnóstico/UI da tentativa atual), `exportStartedAt`/`exportElapsedMs`, `activeFrameIndex`/`totalFrames`/`renderedFrameCount`/`failedFrameIndex`, `lastExportStepDetailed`/`lastSuccessfulExportStep`, `recorderState`, `chunksCount`, `blobSize`, e o erro original **completo** `lastExportOriginalErrorName`/`lastExportOriginalErrorMessage`/`lastExportOriginalErrorStack` (nunca substituído pela mensagem genérica exibida ao usuário). Novos campos `stalledStep`/`stalledElapsedMs`/`stalledFormat`/`stalledResolution`/`stalledFrame`/`stalledRecorderState` para o watchdog de travamento.
- `index.html` (`startRecord`, `exportWithMediaRecorderFallback`): instrumentadas as etapas `export-clicked`, `close-overlays-start/success`, `export-dimensions-ready`, `canvas-created`, `stream-created`, `recorder-created`, `recorder-started`, `render-loop-start`, `render-frame-start/success` (por frame, caminho WebCodecs e fallback MediaRecorder), `render-loop-complete`, `recorder-stop-requested/fired`, `blob-created`, `file-save-start/success`, `cleanup-start/success` — registradas em `arcoExportDiag` via `arcoSetExportDiag`/`arcoTouchExportStep`.
- `index.html`: novo **watchdog de segurança** de 15s — se a etapa atual do export não progride por mais de 15s, registra `stalledStep`/`stalledElapsedMs`/`stalledFormat`/`stalledResolution`/`stalledFrame`/`stalledRecorderState` no diagnóstico, loga `[Arco][export][STALL]` no console e chama `cleanupFailedExport()` para liberar a interface — sem apagar o diagnóstico da tentativa travada.
- `index.html` (`finishExport`/`cleanupFailedExport`): catch do WebCodecs e do fallback MediaRecorder agora preservam `err.name`/`err.message`/`err.stack` completos em `lastExportOriginalError*`, além da mensagem resumida já existente em `lastExportError`.
- `index.html`: novos campos exibidos no painel Diagnóstico (seção de export MP4).
- `index.html`: versionamento atualizado para `v8z4b30G` em comentário do topo, `APP_VERSION`, `APP_VERSION_NAME` e texto visível do app.
- Preserve: nenhuma segunda imagem é renderizada/implementada; Stage, `layoutStage()`, zoom/pan/clamps, curvas, layout aprovado, `projectWorld`/`assets`/`views`/permissões, visuais/ícones/textos/menus/fluxo permanecem inalterados; sem `prompt()`/`alert()`/`confirm()`. Exportador NÃO foi refatorado — apenas instrumentação + watchdog. Formatos 9:16 e 4:3 recebem apenas a mesma instrumentação, sem mudança de comportamento.
