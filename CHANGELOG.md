...
- Base `v8z4b29CD` confirmada antes de qualquer alteração (banner/comentário de topo, `APP_VERSION`, `APP_VERSION_NAME` e texto visível do app conferidos em `v8z4b29CD`).
- `index.html`: versionamento atualizado para `v8z4b30A` em banner/comentário de topo, `APP_VERSION`, `APP_VERSION_NAME` e texto visível do app. Continua versão experimental no repositório de teste — **não promover para estável**.
- feat(scenes): **Cenas V1** — introduz o modelo de projeto com múltiplas cenas (`scenes[]`). Cada cena tem imagem própria e movimento próprio (frames, trechos, curvas, durações, pausas, Tremor por trecho, acabamento/loop). A "cena ativa" é sempre espelhada no estado vivo do app; por isso um projeto de **uma única cena** se comporta exatamente como na `v8z4b29CD` (sem regressão).
- feat(scenes): UI mínima e discreta na Linha do tempo — o antigo rótulo "Cena 1" virou cabeçalho de cena interativo: tocar abre a lista de cenas (trocar), botão "＋" adiciona cena, e o menu traz Duplicar e Excluir. Sempre existe ao menos 1 cena (Excluir desativado quando há só uma).
- feat(scenes): adicionar nova cena pede uma imagem base (cancelar não cria cena vazia), inicia em estado válido com F1 assistido no mesmo formato/aspect do projeto, seleciona a nova cena e atualiza Stage/timeline. Duplicar faz deep copy (editar a duplicata não afeta a original). Excluir seleciona uma cena vizinha válida e nunca deixa `activeSceneIndex` inválido.
- feat(scenes): trocar de cena salva o estado da cena atual e hidrata a nova (Stage, frames, trechos, curvas, durações, pausas, Edição de Tempo), limpando seleções residuais e focando F1 da cena ativa. Stage e timeline mostram apenas dados da cena ativa.
- feat(scenes): Preview e Export concatenam as cenas em sequência (Cena 1 inteira, depois Cena 2, etc.), cada uma com sua imagem/frames; a duração total do projeto é a soma das durações das cenas. Preview e Export usam **a mesma lógica** de sequência (`renderProjectFrame` / `getProjectAnimationDuration`). Acabamento/loop é aplicado por cena conforme a regra atual (loop não cria repetição infinita no meio do projeto, pois a sequência avança para a próxima cena pelo tempo global).
- feat(scenes): JSON salva `scenes[]` (com imagem por cena no save "com imagem") e mantém os campos de topo refletindo a cena ativa para compatibilidade. Ao carregar, JSON antigo sem `scenes[]` é convertido automaticamente em "Cena 1"; JSON novo multi-cena reabre todas as cenas com imagens, frames, trechos, curvas, durações e pausas. Caminho de cena única segue síncrono e leve (sem regressão para o caso comum).
- tweak(ui): rótulo "Tremor Global (projeto inteiro)" em **"TREMOR GLOBAL (PROJETO INTEIRO)"** — caixa alta com estilo técnico equivalente a "INTERVALO PADRÃO" (tracking/peso), alinhado à coluna principal da seção, sem recuo/gap inicial e com o toggle à direita. Texto explicativo abaixo, lógica, estado, preview, export e JSON do Tremor inalterados.
- Não implementado nesta versão (fora do escopo de Cenas V1): textos sobre imagem, camadas/imagens sobrepostas, animação de camadas, transições entre cenas, reordenar/arrastar cenas, renomeação avançada, rotação contínua, nova Home, comunidade/templates.
- Preserve: motor de curva, frames/trechos dentro da cena, Tremor, Movimento Inteligente, Trajetória, Pausa, Duração, Velocidade Constante, Stage, launcher, Novo Projeto e o caminho de Preview/Export/JSON de cena única permanecem com comportamento aprovado da `v8z4b29CD`.
- Validar em iPhone/Safari real (Testes A–I) antes de qualquer estabilização. Não altera o repositório principal.
