...
// v8z4b32E9G1 — a edição manual de largura pelo painel (slider) foi substituída pelas
...
// v8z4b32E9G1-R2 — hardened per revisão: exige a alça REAL visível/interativa (nunca
...
test('E8Z — editor tipográfico e caixa usam fluxo público, isolam draft e bloqueiam o Stage', async ({ page }, testInfo) => {
  ...
  // v8z4b32E9G1 — o controle de largura deixou de existir no painel (nem slider nem
...
test('E9C — Text Asset auto-largura ao conteúdo, modo fixo e paridade sob depth', async ({ page }) => {
  ...
  // (c) Modo fixo (override manual): a E9G1 substituiu o slider pela side width handle
  ...
  // v8z4b32E9G1-R1 — correção de revisão: fixedDraftWidth é uma const do contexto
...
test('E9D — criação pública horizontal e editor tipográfico preserva draft minimizado', async ({ page }) => {
  ...
  // v8z4b32E9G1 — Auto + side width handle: arrastar a alça no Stage entra em fixed e
...
test('E9E — WYSIWYG: painel, Stage, fundo, seleção e alças refletem o mesmo draft', async ({ page }) => {
  ...
  const probe = () => page.evaluate(() => {
    ...
    const panel = {
      ...
      // v8z4b32E9G1 — modo derivado do único botão Auto (ativo=auto, senão fixed); não
  ...
  // 5. Auto → Fixa pela side width handle (v8z4b32E9G1): arrastar a alça no Stage entra
...
test('E9F — editor de texto iconográfico neutro: rail, paleta, coral, gestos e draft', async ({ page }, testInfo) => {
  ...
  // ---------- H) LARGURA Auto + side width handle (v8z4b32E9G1; E9C intacta por baixo) ----------
  ...
  // ---------- L) QUATRO CORNER HANDLES + DUAS SIDE WIDTH HANDLES (E9G1, Text Asset) ----------
  ...
  expect(handleAudit.widthShown).toEqual(['left','right']); // Text Asset ganha exatamente as duas side width handles (E9G1)
...
test('E9F1 — refino do editor de texto: cabeçalho compacto, ícones, paletas, viewport e largura', async ({ page }, testInfo) => {
  ...
  // =========================================================================
  // TESTE 9 — LARGURA AUTO + SIDE WIDTH HANDLE (E9G1: arrastar a alça entra em fixed;
  ...
  // Diagnóstico: o gesto ativou fixed e preservou escala/ancoragem (REG-056/E9G1).
  ...
  // =========================================================================
  // TESTE 10 — E9G1: exatamente quatro corner handles + duas side width handles
...
// v8z4b32E9G1 — gate central da manipulação direta de largura do Text Asset no Stage
...
test('E9G1 — side width handles do Text Asset: largura direta no Stage separada de escala (REG-056)', async ({ page }, testInfo) => {
  test.setTimeout(240_000);
  await page.setViewportSize({ width: 390, height: 797 });
  await page.goto('/', { waitUntil: 'domcontentloaded' });
  await clearStartupStorage(page);
  ...
  await page.evaluate(() => startTextCreation());
  await page.locator('#textCreationInput').fill('Largura E9G1 de teste');
  await page.getByRole('button', { name: 'Confirmar', exact: true }).click();
  ...
  // v8z4b32E9G1-R1 — correção de revisão (Blocker 2): a largura Auto do texto recém-
  ...
  await doCornerDrag();
  // v8z4b32E9G1-R2 — DIAGNÓSTICO OBRIGATÓRIO: lifecycle do corner drag logo após o
  ...
  console.log('[E9G1 diag] afterCornerUp', JSON.stringify(afterCornerUp));
  ...
  console.log('[E9G1 diag] side drag (corner sem Undo)', JSON.stringify(dragNoUndo.diag));
  ...
  console.log('[E9G1 diag] afterUndo', JSON.stringify(afterUndo));
  ...
  console.log('[E9G1 diag] preSideDrag', JSON.stringify(preSideDrag));
  const sideDrag = await dragTextWidthHandleBy(page, 'right', 60);
  console.log('[E9G1 diag] side drag (após Undo do corner)', JSON.stringify(sideDrag.diag));
  ...
  console.log('[E9G1 diag] postSideProductDiag', JSON.stringify({
  ...
  // A última iteração de G2 (boundKind='min') deixa a mutação DIRETA (fora do fluxo
  // público, sem Undo, de propósito — é o próprio setup do gesto no-op) travada no
  // limite lógico MÍNIMO (40): com a fonte desta fixture, isso quebra o texto em
  // dezenas de linhas (worldH >> viewport), empurrando o centro do elemento para fora
  // da área alcançável e sobrepondo chrome não relacionado do Stage. G2 testa
  // clamp/no-op deliberadamente sem tocar Undo; por isso, ao final do bloco, uma
  // mutação direta equivalente (mesmo padrão do próprio setup de G2, não é o
  // comportamento sob teste) restaura uma largura com folga real antes de qualquer
  // interação pública subsequente.
  await page.evaluate(id => {
    const a = assets.find(x => String(x.id) === id);
    const bounds = getTextWidthLogicalBounds();
    const mid = (bounds.min + bounds.max) / 2;
...
// TC-054 / GATE REG-057 — BLOCKER REAL DE MERGE (docs/TEST_CASES.md, docs/REGRESSIONS.md).
// Reedição de um Text Asset já confirmado deve continuar funcionando depois do ciclo
// Editar → Confirmar → Preview → sair do Preview, pelas DUAS rotas públicas de reedição
// (toolbar "Editar"; duplo toque), e também depois de um ciclo que usa a side width
// handle da E9G1. Nenhuma das flags de diagnóstico (pendingTextCreationActive,
// textEditorOpen, textEditorMode, textEditorTargetAssetId) pode ficar presa em estado
// inativo/preso após o ciclo; o asset ID nunca muda; nenhum Text Asset é duplicado.
// A causa raiz histórica da REG-057 NÃO foi comprovada (ver docs/REGRESSIONS.md) — este
// gate não afirma causa, apenas prova o comportamento público exigido.
test('TC-054 — REG-057: reedição pós-Preview pelas duas rotas públicas + ciclo E9G1 com side width drag', async ({ page }) => {
  test.setTimeout(240_000);
  await page.setViewportSize({ width: 390, height: 797 });
  await page.goto('/', { waitUntil: 'domcontentloaded' });
  await clearStartupStorage(page);
  ...
  const editorSnapshot = () => page.evaluate(() => ({
    textEditorOpen: !!pendingTextDraft,
    textEditorMode: textEditorMode || 'none',
    textEditorTargetAssetId: textEditorTargetAssetId || 'none',
    pendingTextCreationActive: !!pendingTextDraft,
    selectedAssetId: selectedAssetId != null ? String(selectedAssetId) : null,
    textCount: assets.filter(a => a && a.type === 'text').length,
    assetTransformDragStateActive: !!assetTransformDragState,
    textWidthDragStateActive: typeof textWidthDragState !== 'undefined' && !!textWidthDragState,
  }));
  const expectEditorFullyClosed = async (label) => {
    const s = await editorSnapshot();
    expect(s, label).toMatchObject({
      textEditorOpen: false, textEditorMode: 'none', textEditorTargetAssetId: 'none',
      pendingTextCreationActive: false, assetTransformDragStateActive: false, textWidthDragStateActive: false,
    });
  };
  const boxOf = (id) => page.locator(`.world-text-asset[data-asset-id="${id}"]`).boundingBox();
  const selectViaTap = async (id) => {
    const box = await boxOf(id);
    await page.touchscreen.tap(box.x + box.width / 2, box.y + box.height / 2);
    expect(await page.evaluate(() => String(selectedAssetId))).toBe(id);
  };
  const openViaToolbar = async (id) => {
    await selectViaTap(id);
    await expect(page.locator('#tbAssetReplace .tb-lbl')).toHaveText('Editar');
    await page.locator('#tbAssetReplace').click();
    await expect(page.locator('#textCreationSheet')).toHaveClass(/open/);
    const s = await editorSnapshot();
    expect(s.textEditorMode).toBe('edit');
    expect(s.textEditorTargetAssetId).toBe(id);
    expect(await page.evaluate(() => textEditorOpenSource)).toBe('toolbar');
  };
  const openViaDoubleTap = async (id) => {
    const box = await boxOf(id);
    const x = box.x + box.width / 2, y = box.y + box.height / 2;
    await page.touchscreen.tap(x, y); // seleciona
    expect(await page.evaluate(() => String(selectedAssetId))).toBe(id);
    await page.waitForTimeout(380); // fora da janela do duplo toque anterior
    ...
    expect(s.textEditorMode).toBe('edit');
    expect(s.textEditorTargetAssetId).toBe(id);
    expect(await page.evaluate(() => textEditorOpenSource)).toBe('double-tap');
  };
  ...
  await page.evaluate(() => startTextCreation());
  await page.locator('#textCreationInput').fill('TC-054 rota 1');
  await page.getByRole('button', { name: 'Confirmar', exact: true }).click();
  const id = await page.evaluate(() => String(getSelectedAsset().id));
  await expectEditorFullyClosed('após criação inicial');
  const initialTextCount = (await editorSnapshot()).textCount;

  // ===== ROTA 1 — toolbar "Editar" =====
  await openViaToolbar(id);
  await expect(page.locator('#textCreationInput')).toHaveValue('TC-054 rota 1');
  await page.locator('#textCreationInput').fill('TC-054 rota 1 — alterado');
  await page.getByRole('button', { name: 'Confirmar', exact: true }).click();
  expect(await page.evaluate(() => String(getSelectedAsset().id))).toBe(id); // mesmo ID
  expect((await editorSnapshot()).textCount).toBe(initialTextCount); // sem duplicação
  await expectEditorFullyClosed('rota 1: após 1ª confirmação');

  await runPreviewCycle();

  await openViaToolbar(id); // reseleciona (canônica) e reabre o editor no MESMO ID
  await expect(page.locator('#textCreationInput')).toHaveValue('TC-054 rota 1 — alterado'); // draft hidratado com o confirmado
  await page.locator('#textCreationInput').fill('TC-054 rota 1 — segunda edição');
  await page.getByRole('button', { name: 'Confirmar', exact: true }).click();
  expect(await page.evaluate(() => String(getSelectedAsset().id))).toBe(id);
  expect((await editorSnapshot()).textCount).toBe(initialTextCount); // ainda sem duplicação
  await expectEditorFullyClosed('rota 1: após 2ª confirmação');

  // ===== ROTA 2 — duplo toque =====
  await runPreviewCycle();
  await openViaDoubleTap(id); // reseleciona e reabre o editor no MESMO ID pela segunda rota pública
  await expect(page.locator('#textCreationInput')).toHaveValue('TC-054 rota 1 — segunda edição');
  await page.locator('#textCreationInput').fill('TC-054 rota 2 — alterado');
  await page.getByRole('button', { name: 'Confirmar', exact: true }).click();
  expect(await page.evaluate(() => String(getSelectedAsset().id))).toBe(id);
  expect((await editorSnapshot()).textCount).toBe(initialTextCount);
  await expectEditorFullyClosed('rota 2: após confirmação');

  await runPreviewCycle();
  await openViaDoubleTap(id); // segunda reedição consecutiva pela rota 2, também após Preview
  await page.locator('#textCreationInput').fill('TC-054 rota 2 — segunda edição');
  await page.getByRole('button', { name: 'Confirmar', exact: true }).click();
  expect(await page.evaluate(() => String(getSelectedAsset().id))).toBe(id);
  expect((await editorSnapshot()).textCount).toBe(initialTextCount);
  await expectEditorFullyClosed('rota 2: após segunda confirmação');

  // ===== GATE REG-057 COM E9G1 ATIVA — Editar → side width drag → Confirmar → Preview
  // → sair do Preview → reabrir o MESMO Text Asset → side width drag novamente →
  // Confirmar → Preview novamente. =====
  // Setup com folga real (item C do escopo): o Text Asset começa no MEIO da faixa
  // lógica disponível para que os DOIS arrastos de aumento tenham espaço real antes do
  // clamp máximo — o teste do clamp/no-op já é coberto separadamente no gate E9G1.
  await page.evaluate(id => {
    const a = assets.find(x => String(x.id) === id);
    const bounds = getTextWidthLogicalBounds();
    const mid = (bounds.min + bounds.max) / 2;
  ...
  await openViaToolbar(id);
  await page.getByRole('tab', { name: 'Largura da caixa', exact: true }).click();
  const widthBeforeDrag1 = await page.evaluate(() => pendingTextDraft.boxWidth);
  await dragTextWidthHandleBy(page, 'right', 50);
  expect(await page.evaluate(() => pendingTextDraft.boxWidthMode)).toBe('fixed');
  const widthAfterDrag1Draft = await page.evaluate(() => pendingTextDraft.boxWidth);
  expect(widthAfterDrag1Draft).not.toBeCloseTo(widthBeforeDrag1, 0);
  const undoBeforeConfirm1 = await page.evaluate(() => undoStack.length);
  await page.getByRole('button', { name: 'Confirmar', exact: true }).click();
  expect(await page.evaluate(() => String(getSelectedAsset().id))).toBe(id);
  expect(await page.evaluate(() => undoStack.length)).toBe(undoBeforeConfirm1 + 1); // exatamente 1 Undo
  const widthAfterConfirm1 = await page.evaluate(id => assets.find(a => String(a.id) === id).boxWidth, id);
  expect(Math.abs(widthAfterConfirm1 - widthAfterDrag1Draft)).toBeLessThan(1.5); // âncora/largura do draft preservada
  expect((await editorSnapshot()).textCount).toBe(initialTextCount);
  await expectEditorFullyClosed('gate E9G1: após 1ª confirmação de width drag');

  await runPreviewCycle();

  await openViaToolbar(id); // reseleciona e reabre o editor no MESMO ID depois do ciclo com width drag + Preview
  await page.getByRole('tab', { name: 'Largura da caixa', exact: true }).click();
  const widthBeforeDrag2 = await page.evaluate(() => pendingTextDraft.boxWidth);
  expect(Math.abs(widthBeforeDrag2 - widthAfterConfirm1)).toBeLessThan(1.5); // 2º drag parte do estado confirmado no 1º
  const widthHandlesFunctional = await page.evaluate(() => document.querySelectorAll('.text-width-handle.show').length);
  expect(widthHandlesFunctional).toBe(2); // side handles continuam funcionais depois do Preview
  await dragTextWidthHandleBy(page, 'right', 40);
  const widthAfterDrag2Draft = await page.evaluate(() => pendingTextDraft.boxWidth);
  expect(widthAfterDrag2Draft).not.toBeCloseTo(widthBeforeDrag2, 0);
  await page.getByRole('button', { name: 'Confirmar', exact: true }).click();
  expect(await page.evaluate(() => String(getSelectedAsset().id))).toBe(id); // ID permanece o mesmo em todo o ciclo
  expect((await editorSnapshot()).textCount).toBe(initialTextCount); // nenhuma layer duplicada
  await expectEditorFullyClosed('gate E9G1: após 2ª confirmação de width drag');

  await runPreviewCycle(); // Preview continua funcionando após o ciclo completo

  const finalSnapshot = await editorSnapshot();
  expect(finalSnapshot.selectedAssetId).toBe(id); // seleção canônica ainda aponta para o mesmo Text Asset
  expect(finalSnapshot.assetTransformDragStateActive).toBe(false); // nenhum estado de gesto fica preso
  expect(finalSnapshot.textWidthDragStateActive).toBe(false);
});
