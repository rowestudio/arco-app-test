...
test('E9D — criação pública horizontal e editor tipográfico preserva draft minimizado', async ({ page }) => {
  ...
  // Preview aberto publicamente: snapshot e auditoria usam a geometria canônica,
  // e a contribuição física do texto ocupa uma região horizontal no canvas.
  ...
  const previewProof=await page.evaluate(id=>{if(animFrame)togglePreviewPlayback();const canvas=document.getElementById('previewDisplayCanvas'),ctx=canvas.getContext('2d'),durationSec=getComputedTimelineDuration(),totalPF=Math.max(1,Math.round(durationSec*25)),source=getPreviewRenderSource(),snap=renderSessionSnapshot?.textAssets?.find(a=>String(a.id)===id),audit=renderTransform.preview?.assets?.find(a=>String(a.id)===id),original=renderSessionSnapshot;renderFrameSafely(ctx,canvas,0,canvas.width,canvas.height,totalPF,0,{renderSource:source});const withText=ctx.getImageData(0,0,canvas.width,canvas.height).data,noText=document.createElement('canvas');noText.width=canvas.width;noText.height=canvas.height;renderSessionSnapshot={...original,textAssets:original.textAssets.filter(a=>String(a.id)!==id)};renderFrameSafely(noText.getContext('2d'),noText,0,noText.width,noText.height,totalPF,0,{renderSource:source});renderSessionSnapshot=original;const base=noText.getContext('2d').getImageData(0,0,noText.width,noText.height).data;let minX=canvas.width,minY=canvas.height,maxX=-1,maxY=-1,changed=0;for(let i=0;i<withText.length;i+=4){if(Math.abs(withText[i]-base[i])+Math.abs(withText[i+1]-base[i+1])+Math.abs(withText[i+2]-base[i+2])+Math.abs(withText[i+3]-base[i+3])>20){const p=i/4,x=p%canvas.width,y=Math.floor(p/canvas.width);minX=Math.min(minX,x);maxX=Math.max(maxX,x);minY=Math.min(minY,y);maxY=Math.max(maxY,y);changed++}}return{snap,audit,changed,bounds:{w:maxX>=minX?maxX-minX+1:0,h:maxY>=minY?maxY-minY+1:0}}},String(textDraftId));
  expect(previewProof.snap).toMatchObject({id:textDraftId,text:'Texto',worldW:stageProof.asset.worldW,worldH:stageProof.asset.worldH,depth:42}); expect(previewProof.audit).toMatchObject({id:textDraftId,drawn:true,intersectsCamera:true}); expect(previewProof.audit.screenW).toBeGreaterThan(previewProof.audit.screenH); expect(previewProof.changed).toBeGreaterThan(0); expect(previewProof.bounds.w).toBeGreaterThan(previewProof.bounds.h); await page.locator('#previewScreen .close-btn').click();
});
