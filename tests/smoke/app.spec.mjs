...
test('E8Y — editor tipográfico usa fluxo público, isola draft e bloqueia o Stage', async ({ page }, testInfo) => {
  ...
  await page.evaluate(async()=>{const cp=await readSessionCheckpoint();await restoreLastSessionAutosave(cp)}); expect(await page.evaluate(id=>serializeProjectAsset(assets.find(a=>String(a.id)===id),0,false),String(id))).toEqual(postDrag.canonical);
  ...
  await page.evaluate(async()=>{const cp=await readSessionCheckpoint();await restoreLastSessionAutosave(cp)}); expect(await page.evaluate(id=>serializeProjectAsset(assets.find(a=>String(a.id)===id),0,false),String(id))).toEqual(committed.asset);
