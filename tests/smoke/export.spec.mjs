...
test('WebKit macOS real Export — imagem com Text Asset E8Y editado', async ({page}, testInfo) => {
  ...
  await page.getByRole('button',{name:'Fonte',exact:true}).click(); await page.getByRole('button',{name:'Fonte Serifada',exact:true}).click();
  await page.getByRole('button',{name:'Estilo',exact:true}).click(); await page.getByRole('button',{name:'Negrito + itálico',exact:true}).click();
  await page.getByRole('button',{name:'Alinhar',exact:true}).click(); await page.getByRole('button',{name:'Direita',exact:true}).click();
